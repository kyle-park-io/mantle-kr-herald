...
import type { ArticleBlock, ArticleBody, CollectedThread, SourceTweet, UserProfile } from "../../src/domain/models";
...
class FakeGateway implements SourceGateway {
  ...
  async fetchArticle(tweetId: string): Promise<ArticleBody | undefined> {
    ...
    const blocks = this.articles[tweetId];
    // Absent from the map means the endpoint has no article for that tweet, which it answers with
    // a null `article` — a different fact from an article that came back with no usable blocks.
    return blocks === undefined ? undefined : { title: "stub", blocks };
  }
