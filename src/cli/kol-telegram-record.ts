...
import { loadGoogleAuthConfig, loadGoogleSheetConfig, loadDbConfig } from "../config";
...
try {
  ...
  const renderings = await createStores(db).formattingStore.loadAll();
  ...
  //
  // The database is named in the message because the likeliest way to get a real zero here is
  // pointing at the wrong one — a fresh local/dev database has no renderings at all, and the count is
  // 0 for a reason that has nothing to do with the month being swept.
  if (candidates.length === 0) {
    console.warn(
      `[kol-telegram] no approved Telegram rendering to attribute against (read ${renderings.length} rendering(s) from the database) — ` +
        `every row's itemId, matchScore and topic will be blank, and a human fills topic by hand. ` +
        `Expected for a month whose posts predate our first approved copy; unexpected if this is pointed at a fresh ` +
        `local/dev database with no formatted renderings in it yet.`,
    );
  }
