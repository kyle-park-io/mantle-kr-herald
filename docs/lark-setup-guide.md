# Lark 셋업 가이드 — App ID·Secret·chat_id 발급/확인하기

> 이 문서는 서브시스템 B(Lark 데이터 수집)가 필요로 하는 값들을 **어디서 발급받고 어떻게
> 확인하는지** 단계별로 설명합니다. 최종적으로 `.env`에 아래 4개를 채우는 게 목표예요:
> `LARK_APP_ID`, `LARK_APP_SECRET`, `LARK_CHAT_IDS`, (선택) `LARK_BASE_URL`.
>
...
| 스코프 | 용도 | 필수? |
| --- | --- | --- |
| `im:message.history:readonly` | 그룹 **과거 메시지** 읽기 (수집 런타임) | ✅ 필수 |
| `im:chat:readonly` | 봇이 속한 **그룹 목록·chat_id** 조회 (chat_id 확인용) | 권장(§6에서 사용) |

> `im:message:readonly`도 메시지 읽기를 커버합니다. 히스토리 조회에는
> `im:message.history:readonly`가 가장 정확합니다.
