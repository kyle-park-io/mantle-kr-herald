...
**④ CLI는 `npx vercel`로만 씁니다 — 전역 설치하지 마세요.** 이 문서와 `deploy.md`의 vercel 명령은
전부 `npx vercel …`이고, `pnpm deploy:check`도 **`npx`로 스폰합니다**
...
| `[local]` 발행 버튼 | 로컬 대시보드·CLI만 | 함수의 파일시스템(`/var/task`)은 읽기 전용이고, 써진들 그 파일을 돌려줄 라우트(`GET /api/publish/local/*`)가 호스팅에는 없습니다. `availableTargets`에서 아예 빠지므로 **버튼이 비활성**이고, 그래도 `local`을 지정해 보낸 요청은 200에 실패를 숨기지 않고 **500으로 거부**합니다. `google`/`lark` 발행은 그대로입니다 |
