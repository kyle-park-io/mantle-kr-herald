...
합니다) 확인하고, 로그아웃 뒤 `/api/status`가 다시 401을 돌려주는지로 세션이 실제로 끊겼는지
확인합니다.

기본 실행은 로그인 잠금까지는 건드리지 않습니다. `--lockout`을 붙이면 같은 주소로 다섯 번
연속 로그인을 실패시켜 60초 잠금이 실제로 걸리는지까지 확인하지만, **그 60초 동안은 이
주소에서 아무도(운영자 포함) 로그인할 수 없으므로** 팀이 대시보드를 쓰고 있지 않을 때만
붙이세요.

> **`deploy:check`도 `deploy:smoke`도 배포된 Google refresh 토큰이 살아 있는지는 확인하지
> 못합니다.** 애플리케이션이 검사하는 것은 자격 증명이 "있는지"이지 "동작하는지"가 아니라서,
> 토큰이 폐기돼도 `google`은 여전히 발행 타깃 목록에 남습니다 — 개발 중 실제로 `doctor`가
> "Google Drive configured"라고 보고한 한 시간 내내 매 refresh가 `invalid_grant`를 반환한 적이
> 있습니다. 토큰을 로테이션했다면 이 목록에 있다는 사실만으로 안심하지 말고 실제로 한 번
> 발행해 보세요.
