...
발송은 그 숫자에서 아직 빠지지 않은 상태입니다. 발송 게이트 자체는 이런 예약 건도 내부적으로
차감해서 판단하므로 쿼터를 넘겨 보내는 사고는 나지 않지만, 화면 숫자만 보고 "잔여 N건이니 N건을
더 보낼 수 있다"고 판단하면 안 됩니다 — 직전에 다른 실행이 예약을 넣었다면 실제 여유는 그보다
적습니다.
