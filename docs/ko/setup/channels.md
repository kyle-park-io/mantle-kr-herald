...
- **채널(브로드캐스트):** 봇을 **관리자(Administrators) 추가** → **"메시지 게시(Post messages)" 권한**을
  켜야 발송됩니다.
- **핀(고정)까지 쓰려면:** 위 발송 권한과 별개로 봇이 그 방의 **관리자**여야 합니다 — 그룹은
  **"메시지 고정(Pin messages)" 권한**, 채널은 **"메시지 수정(Edit messages)" 권한**을 켜야 고정됩니다.
  없어도 발송 자체는 그대로 나가고, **고정만** 실패해 "글은 올라갔지만 고정하지 못했습니다"로 알려줍니다.
- **⚠️ 일반 그룹에서 봇을 관리자로 올리면 그 방의 `chat_id`가 바뀝니다.** 텔레그램이 그 순간 일반 그룹을
  **슈퍼그룹으로 승격**시키기 때문입니다(id가 `-123456789` → `-100…`으로 바뀜). 옛 id로는 발송이
  `HTTP 400 — group chat was upgraded to a supergroup chat`으로 거절되고, **그 응답의
  `parameters.migrate_to_chat_id`가 새 id입니다.** T-3으로 돌아가 그 방의 `TELEGRAM_CHAT_ID_*`를
  새 값으로 바꾸세요. 이미 슈퍼그룹이거나 채널이면 승격이 일어나지 않으니 id도 그대로입니다.
...
**`pnpm send:x-article`도 이제 같은 보정된 숫자를 기준으로 게이트를 거칩니다.** 아티클 원장
(`x-article.json`)도 배너와 두 게이트(`send:channels`, `send:x-article`)가 예약분을 셀 때 함께 읽으므로,
방금 그 명령으로 예약한 아티클은 보정된 숫자와 다음 실행의 게이트 판단에 곧바로 반영됩니다. 이번
실행이 필요로 하는 건수가 보정된 숫자보다 많으면 `send:x-article`도 — `send:channels`와 마찬가지로 —
한 건도 나가지 않고 전량 보류됩니다.
...
- **멱등(중복 발송 방지):** `output/publish/deliveries.json`이 `(아이템:타입:방)`별 발송 이력을 기록 —
  ...
  - **`예약 취소됨`(`status: "dropped"`) 행은 예외로, 다시 발송 대상이 됩니다.** X 예약분의 초안이
    게시 전에 지워지면 그 방에는 아무것도 도착하지 않았으므로, `pnpm send:reconcile`이 그 행을 물러나게
    한 뒤에는 재실행이 그 항목을 **다시 보냅니다** — 의도한 동작입니다. 즉 보장되는 것은 "재실행이
    아무 일도 하지 않는다"가 아니라 "**실제로 도착한 글은 두 번 나가지 않는다**"입니다.
