...
**`pnpm send:x-article`도 이제 같은 헤드룸으로 게이트됩니다.** 아티클 원장(`x-article.json`)도 배너와
두 게이트(`send:channels`, `send:x-article`)가 예약분을 셀 때 함께 읽으므로, 방금 그 명령으로 예약한
아티클은 배너의 보정된 숫자와 다음 실행의 게이트 판단에 곧바로 반영됩니다. 이번 실행이 필요로 하는
건수가 남은 헤드룸보다 많으면 `send:x-article`도 — `send:channels`와 마찬가지로 — 한 건도 나가지
않고 전량 보류됩니다.
