...
남은 건수는 `pnpm doctor --live`와 대시보드 2차 검수(발송판) 배너에서 각각 확인할 수 있는데, 이제 둘이
보여주는 숫자의 **의미가 다릅니다**:

```
✓ Typefully  live  publishing quota 6 left of 15 on the account · resets 2026-08-01
```

`doctor`는 **계정에 남은 순수 건수**를 그대로 보여줍니다 — "이 연동이 건강한가"를 답하는 화면이라, 지금
당장 몇 건을 더 보낼 수 있는지는 답하지 않습니다. 반면 대시보드 배너는 그 숫자에서 **예약(스케줄)됐지만
아직 게시로 확인되지 않은 건**을 미리 빼서 보여줍니다 — `X 발행 잔여 4건 (예약 2건 대기) / 15건 ·
08/01 리셋`처럼, 앞의 큰 숫자가 `pnpm send:channels`의 발송 게이트가 실제로 허용하는 건수입니다. 예약
대기가 없으면 `(예약 N건 대기)` 부분은 아예 붙지 않습니다. 이 보정된 숫자가 3건 이하로 떨어지면 배너가
amber로 바뀌고, 쿼터를 읽지 못하면(오류 등) 배너 자체가 뜨지 않습니다(0건으로 잘못 표시되는 일은
없습니다).
...
**단, `pnpm doctor --live`의 숫자만 보고 판단하지 마세요.** 위에서 말했듯 doctor는 예약 대기를 빼지
않은 계정 원본 숫자라, 몇 분 전에 다른 실행이 예약을 넣었다면 doctor가 보여주는 수보다 실제로 더 보낼
수 있는 건 적습니다. `pnpm send:channels`의 발송 게이트 자체는 이런 예약 건도 내부적으로 차감해서
판단하므로 **그 경로로는** 쿼터를 넘겨 보내는 사고가 나지 않지만, "doctor에 6이라고 떴으니 6건 더
보낼 수 있다"고 판단하면 안 됩니다 — 보정된 숫자가 필요하면 대시보드 배너를 보세요.

**`pnpm send:x-article`는 이 보정 어디에도 잡히지 않습니다.** 아티클이 예약되는 곳은 `SendChannels`가
보는 발송 원장(`deliveries.json`)이 아니라 별도의 `x-article.json`이라, 방금 그 명령으로 예약한
아티클은 doctor의 원본 숫자에도, 배너의 보정된 숫자에도, 게이트의 `used`·`remaining`·예약분 계산
어디에도 잡히지 않습니다. `send:x-article`을 막 돌린 직후 같은 계정으로 다른 X 배치
(`send:channels --target x`)를 시작한다면, 두 화면의 숫자를 그대로 믿지 말고 실제 여유를 별도로
가늠하세요.
