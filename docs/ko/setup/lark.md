...
> **공유 창에서 앱 이름이 검색되지 않습니다 — 대신 봇이 들어있는 그룹 채팅을 추가하세요.**
> 폴더 → 공유 → 협업자 검색에 앱 이름(예: `Kyle's AI Assistant`)을 쳐도 결과에 **안 뜹니다**.
> Lark 공유 다이얼로그는 사용자·그룹만 검색하고 앱 자체는 노출하지 않습니다. 해결책은
> **§5에서 봇을 초대해 둔 그룹 채팅(`oc_…`)을 협업자로 추가**하는 것 — 그룹에 부여한 권한을
> 그 안의 봇이 물려받아 폴더에 접근하게 됩니다. review/approved 두 폴더 모두에 **편집 가능**으로
> 추가하세요.
>
> 스코프를 더 켜도 해결되지 않습니다. `drive:drive`·`drive:file`·`contact:*`를 전부 승인받은
> 상태에서도 검색에는 안 뜹니다 — 스코프는 *API 호출 권한*이고, 공유 창 검색은 *주소록 조회*라
> 층이 다르기 때문입니다. 앱은 주소록 개체가 아닙니다.
>
> 앱 이름이 헷갈리면 개발자 콘솔 대신 API로 확인하는 게 확실합니다:
> `GET /open-apis/bot/v3/info` → `app_name`. 콘솔 목록의 이름과 `LARK_APP_ID`가
> 실제로 같은 앱인지 대조할 때 씁니다.

> **⚠️ 권한이 그룹에 묶입니다.** 이 방식은 폴더 접근권을 그룹 멤버십에 연결하므로, 그 그룹에
> 초대되는 사람은 누구나 review/approved 폴더에 접근하게 됩니다. 아무나 드나드는 테스트 채팅
> 말고 **전용 그룹**을 쓰세요.
...
| `HTTP 403 code 1061004 forbidden` | 스코프는 있는데 **폴더가 봇에게 공유 안 됨**. 스코프 문제였다면 `99991672`가 났을 것 — 즉 §10-1이 아니라 **§10-2**(봇 그룹을 협업자로 추가)를 확인하세요 |
