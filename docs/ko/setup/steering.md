...
> **`conversion/x.md`만은 프롬프트로 들어가지 않습니다.** `x`는 1차에서 승인된 번역이 그대로 X
> 게시물이 되므로 변환 단계 자체가 없고, 그 파일은 번역자와 2차 검수자가 읽는 기준 문서로 남습니다.
> 그래도 위 꾸러미에는 그대로 들어갑니다 — 사람이 읽어야 하는 문서라는 점은 달라지지 않았고,
> `pnpm doctor`도 여전히 그 파일이 있는지 봅니다.
...
> 반대로 하면 넣을 테이블이 없습니다. 복원은 **스코프 단위 교체**입니다 — 스냅샷에 들어 있는
> 스코프는 기존 행을 지운 뒤 스냅샷 순서 그대로 다시 넣습니다. 그래서 여러 번 돌려도 코퍼스가
> 부풀지 않고, **순서까지 스냅샷 그대로**입니다(`ordinal` 순서가 곧 프롬프트에 들어가는 순서라
> 순서가 어긋나면 모델이 보는 내용이 조용히 달라집니다). 대신 로컬에만 있던 예시는 이때
> 사라지므로, 개발 DB에 직접 넣어 둔 예시가 있다면 복원 전에 챙기세요. 스냅샷에 없는 스코프는
> 건드리지 않습니다.
