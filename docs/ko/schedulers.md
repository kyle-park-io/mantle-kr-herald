...
**이 문서는 목록입니다.** 이 저장소에는 systemd 사용자 타이머가 일곱 개 있고, 다섯 개는
[`team-runbook.md`](team-runbook.md) §6에, `herald-backup`(파이프라인이 아니라
스티어링·프로덕션 상태 백업)은 [`setup/steering.md`](setup/steering.md) §6에, 나머지 하나
(`herald-kol-weekly`)는 [`capabilities.md`](capabilities.md) §5-P에 자세한 설명이
...
| `herald-kol-weekly` | `Tue *-*-* 07:30:00` | **주 1회, 화요일.** `pnpm kol:quarter` — 분기의 세 달 모두를 매번 처음부터 다시 훑어(부분 이어달리기가 아님) `KOL list` 로스터·`kol-telegram-posts`·월별 로그를 최신화합니다. 로그 영역의 **값 일곱 열만** 씁니다 — 요약 블록(`Posts`/`Views`/`Total Cost` 등 기존 수식)·`followers`·세 비율 컬럼(`Engagement Rate`/`Cost per impression`/`Duplicated?`)은 절대 안 건드립니다. `--notify`가 없어 계약 대비 부족분·매칭 안 된 계약 이름·절단된 채널은 어디로도 안 보내지고 실행 로그에만 남습니다 — 유일한 알림은 실행 자체가 죽었을 때의 `OnFailure=`뿐입니다. | [§5-P KOL 분기 딜리버러블 트래킹](capabilities.md) |

분 단위가 전부 다른 것(`:07/:37`, `:17`, `:41`, `:23`, `:30`, `:47`, `:53`)은 우연이 아닙니다.
일곱 유닛이 같은 배포 체크아웃에서 같은 데이터베이스나 같은 워크북을 향해 `pnpm`을 돌리고 그중
둘은 `claude -p`까지 띄우므로, 분을 나눠 쓰면 잃을 게 없고 겹치면 잠든 Neon 컴퓨트 위에서 두 실행이
동시에 깨어납니다. 각 타이머 파일이 서로의 분을 피해 적혀 있고, `tests/deploy/credsTiming.test.ts`와
`tests/deploy/translateCheckTiming.test.ts`가 `deploy/`에서 그 분들을 다시 읽어 충돌을 막습니다.

## 일곱이 공유하는 구조

유닛 파일을 하나 열어 본 적이 있다면 나머지 여섯은 거의 같은 모양입니다.
...
- **`ExecStart=… deploy/herald-run-logged.sh %n …`** — 모든 명령이 래퍼를 거칩니다. 이 머신의
  ...
  **그대로** 돌려주므로 위 실패 알림이 살아 있습니다. **기기에 따라 이 창은 몇 달이기도 합니다**
  (판별법은 `team-runbook.md` §4 "…시계") — 그래도 래퍼는 그대로 둡니다. 시계는 다시 틀어질 수
  있고, 파일에는 회전 창이라는 게 애초에 없습니다.
...
- **`loginctl enable-linger`** — 유닛 파일이 아니라 **머신 설정**이고, 기본값이 꺼짐입니다. 꺼져
  있으면 사용자 유닛은 로그인해 있는 동안만 살아서, 마지막 셸을 닫는 순간 일곱 타이머가 같이
  사라집니다. "분명히 켰는데 `list-timers`가 비어 있다"는 대개 이것입니다
  ([§6 설치 7번](team-runbook.md#설치)).

설치 파일 목록(서비스·타이머 열네 개와 공용 실패 훅 하나)은 **여기에 옮겨 적지 않습니다.** 저장소에
...
무엇을 하는지만큼 중요한 목록입니다. 이 일곱 유닛이 사람 대신 결정하는 일은 없습니다.
...
- **텔레그램 알림은 예외이고, 그건 발송이 아닙니다.** 일곱 유닛 전부 실패하면 ops 방으로 알림을
...
- 각 스케줄러의 설치·확인·정지, 배치 크기 조정, 사고 대응 → [`team-runbook.md`](team-runbook.md) §6
  ([일곱 개를 한꺼번에 멈추는 법](team-runbook.md#멈추기)도 거기 있습니다 — `stop`은 글롭을 받고
  `disable`은 받지 않습니다)
