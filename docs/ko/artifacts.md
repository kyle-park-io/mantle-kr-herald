...
(`glossary.json`, `glossary-dismissed.json`, `style-guide.md`, `locale.json`, `tm.json`, 타입별
`conversion/<타입>.md`)는 `.gitignore`로 제외됩니다. `pnpm config:init`이 예시 파일을 복사해 실제
파일을 만들어 줍니다 (§3 참고).
...
**저장 모드를 실제로 읽어서 검사하는 명령은 소수뿐입니다** — 아래 표의 열한 개 CLI(`drive:init`/
`sheet:init`/`targets:list`/`history:record`/`impressions:record`/`metrics:record`/
`kol-telegram:record`/`x:reconcile`/`x:link`/`kol:quarter`/`kol-roster-migrate`), `pnpm doctor`,
`pnpm drive:publish`, `pnpm serve`가 전부입니다.
...
토큰뿐입니다), 모드가 없거나 잘못돼도 이들에는 애초에 영향이 없습니다.

`pnpm status`도 저장 모드를 안 읽는 쪽이지만 이유가 다릅니다 — 클라우드 명령이 아니라 읽기 전용
진단이므로, `src/cli/status.ts`는 `parseStorageMode`도 `tryParseStorageMode`도 import하지 않습니다.
**그렇다고 파일을 읽는 것은 아닙니다** — 집계는 전부 `createStores(db)`를 거친 데이터베이스
조회입니다. 그래서 모드가 없거나 잘못 설정돼 있어도 멈추지 않지만, `DATABASE_URL`이 없거나 스키마가
아직 없으면 실패합니다. 모드에 관해서는 아래 표처럼 `local`과 `cloud`에서 정확히 동일하게 경고를
표시합니다.

| | `local` | `cloud` |
|---|---|---|
| `collect` → `translate` → `convert` → `format` | 동일하게 동작 | 동일하게 동작 |
| `drive:init`, `sheet:init`, `targets:list`, `history:record`, `impressions:record`, `metrics:record`, `kol-telegram:record`, `x:reconcile`, `x:link`, `kol:quarter`, `kol-roster-migrate` (열한 개) | `"<command>: local mode — skipped (set HERALD_STORAGE_MODE=cloud to enable)"`을 출력하고 종료 코드 `0` | 정상 실행 |
...
깨뜨릴 수 있기 때문입니다. 이 게이트는 위 아홉 개 CLI가 공통으로 호출하는
`skipIfLocal()`(`src/cli/skipIfLocal.ts`)로 구현되어 있으며, 정확한 목록은 그 호출부 자체입니다
(`grep -rn skipIfLocal src/cli`). `publish.ts`는 이 목록에 없습니다 —
...
> **이 표의 `output/**.json` 이름을 파일로 읽지 마세요.** 2026-07-31 hosted-writes 전환 이후
> 파이프라인 원장 — 수집 스레드, 번역, 변환본, 렌더링, 방별 포크, 발송·동기화 원장, 리니지, 그리고
> few-shot 코퍼스 — 은 전부 **Postgres 테이블**이고, 명령은 `createStores(db)`(`src/cli/stores.ts`)를
> 거쳐 `Pg*Store`로 읽고 씁니다. `output/` 트리는 이제 `pnpm db:export`/`pnpm db:import`의 **교환
> 형식**일 뿐입니다(`.vercelignore`의 같은 설명 참고). 아래 이름들은 그 교환 형식에서의 파일명이자
> 각 원장의 통용 이름으로 남겨둔 것이며, 어느 테이블을 말하는지 가리키는 라벨로 읽으세요.
>
> **여전히 진짜 파일인 것**은 따로입니다 — 워크시트(`output/*/worksheets/`),
> `pending.json` 세 개, 수집 워터마크(`output/x/state.json`·`output/lark/state.json`),
> collect 실행 로그(`output/x/runs.json`), `output/archive/`, `output/publish/local/`,
> 그리고 루트의 스티어링 설정(`translation/`·`conversion/`).
...
| 대시보드 `링크 수집` 탭 — `POST /api/intake/x` (`GET /api/intake/pending`은 대기 목록 조회, 아래 참고) | `TWITTERAPI_IO_KEY`(env — 없으면 `createDeps`가 `collectLinkedThread` 의존성을 아예 만들지 않고, `POST`는 그 부재를 보고 400으로 거부됨); 요청 본문의 URL이 가리키는 트윗 id 하나의 스레드 전체(twitterapi.io `fetchThread`, 아티클이면 `fetchArticle`로 본문·제목·표지까지 — `fetchThread`가 읽는 `thread_context`는 `advanced_search`와 달리 `article` 필드를 아예 주지 않으므로, 아티클 판별은 **트윗 본문이 `x.com/i/article/<번호>` 주소 하나뿐인지**로 하고 그 응답이 제목·표지의 유일한 출처입니다. 스레드 첫 글이 아티클인데 본문을 못 읽으면 저장 전에 거절 — 원문이 링크 한 줄만 남기 때문); 이미 수집됐는지 판정을 위한 수집 저장소(`x_threads`) 전체; 이미 번역됐는지 판정을 위한 번역 원장(`translations`); 번역 기준 시각 판정을 위한 스케줄러의 마지막 보고(`translate_floor_reports` 한 행 — 번역 선별과 **같은 함수**(`meetsTranslateFloor`)가 아니라고 답하면 저장 전에 거절: `Mantle_Official`의 기준 시각 이전 글, 그리고 작성자를 못 읽은 기준 시각 이전 글) | `x_threads`에 스레드 **한 행**만 upsert(재수집 안전 — 이미 있던 스레드면 본문·미디어만 갱신). **`output/x/state.json`(수집 워터마크)도 `output/x/runs.json`(실행 로그)도 건드리지 않습니다** — 링크 수집은 계정 타임라인을 훑지 않으므로 "어디까지 읽었나"를 바꿀 이유가 없고, 이 정기 수집과의 접점을 남기지 않는 덕에 이 경로는 파일시스템을 전혀 만지지 않아 읽기 전용 FS인 Vercel 함수 안에서도 그대로 동작합니다. `GET /api/intake/pending`은 위 중 `x_threads`+`translations`+`translate_floor_reports`만 **읽기만** 합니다 — 마지막 하나를 읽는 이유는 대기 목록이 "틱이 집어갈 것"과 같아야 하기 때문으로, 기준 시각 이전이라 어느 틱도 집어가지 않을 `Mantle_Official` 글은 번역 선별과 **같은 함수**(`meetsTranslateFloor`)로 목록에서 뺍니다(보고가 없거나 기준 시각 없이 돌았다는 보고면 아무것도 안 뺍니다). `TWITTERAPI_IO_KEY` 없이도 동작(대기 목록은 키가 없어도 보여야 하므로) | twitterapi.io API(`POST`만 — `GET .../pending`은 호출 없음) |
...
| `pnpm translate:prepare [--source x\|lark] [--ids] [--since] [--limit]` | `output/x/items.json`, `output/lark/items.json`(`--source`로 한쪽만 선택 가능); 이미 번역된 id 제외를 위한 `output/translations/translations.json`; `translation/glossary.json`; few-shot 코퍼스(`few_shot_examples` 테이블 — 파일이 아니라 DB); `translation/tm.json`(번역 메모리 — 큐레이션 few-shot에 더해 이 배치에 관련도 높은 쌍만 골라 인라인), `translation/style-guide.md`, `translation/locale.json` | `output/translations/worksheets/`에 `batch-<타임스탬프>.md` 워크시트 생성; `output/translations/pending.json` 갱신(덮어쓰기 전 이전 배치를 `output/archive/<YYYY-MM-DD>/`로 자동 이동) | 없음 |
| `pnpm translate:save --id --file [--approve]` | `output/translations/pending.json`(없으면 `output/translations/translations.json`에서 이미 저장된 항목으로 폴백); `--file`로 지정한 로컬 한글 텍스트 | `output/translations/translations.json`(upsert); `--approve` 시 few-shot 코퍼스(`few_shot_examples` 테이블)에 예시 추가(원문이 너무 길면 승격 생략) | 없음 |
...
| `pnpm translate:check [--status <s>] [--since <ISO>] [--published] [--notify]` | 번역 원장 `output/translations/translations.json`(`--status`/`--since`로 필터링 가능); `translation/glossary.json`(용어집 — 비어 있으면 모든 검사가 무의미하게 통과해 버리므로, 실행 자체를 거부하고 대신 에러로 멈춤). 기본은 각 행의 초안을 검사하고, `--published`면 실제 발행 원문이 채워진 행만 골라 그 원문을 검사(원문이 없는 행은 건너뛰고 몇 건 건너뛰었는지 함께 보고). `--notify`면 `TELEGRAM_BOT_TOKEN`+`TELEGRAM_CHAT_ID_OPS`(env, 둘 다 있어야 함 — 없으면 조용히 안 보냄) | 없음 — 표준 출력만(용어집 드리프트 목록과, `--published` 여부와 무관하게 항상 함께 나오는 오버라이드 목록). 읽기 전용이라 드리프트나 오버라이드가 나와도 종료 코드는 그대로 `0` | 없음. `--notify`이고 **오버라이드가 하나라도 있을 때만** Telegram Bot API(`sendMessage`) 1회 — 용어별로 결정 표현과 몇 개 항목에서 뒤집혔는지만 보내며(글 본문·항목 id는 안 보냄), 드리프트는 절대 알림을 띄우지 않습니다(사람이 읽고 판단할 목록이라 알림으로 띄우면 무시하게 됩니다). 알림이 실패해도 종료 코드는 `0` |
| `pnpm collect:reference [target] [--since <3d\|12h\|1w\|ISO>] [--limit <n>]` | `TWITTERAPI_IO_KEY`(env); 참고 계정 핸들 `REFERENCE_X_HANDLE`(env, 기본 `0xMantleKR`); 기존 스레드 병합을 위한 `output/x/reference/items.json`; 워터마크 조회를 위한 `output/x/reference/state.json` | `output/x/reference/{items,state,runs}.json`(`collect`과 동일한 엔진, **소스 스토어(`output/x/`)와 격리** — 한국어 완성본이 번역 큐에 절대 안 섞임). 손으로 돌리는 명령이라 **개발 체크아웃**에 쓰입니다 — 스케줄러가 읽는 `~/.herald/output/x/reference/`로는 `bash deploy/herald-deploy.sh`의 4단계(`deploy/herald-copy-corpus.sh`)가 배포 시점에 복사합니다 | twitterapi.io API |
...
| `pnpm convert:prepare [--ids] [--since] [--limit] [--types] [--max-variants]` | 승인된 항목을 위한 `output/translations/translations.json`; 이미 변환된 키 제외를 위한 `output/variants/variants.json`; `translation/glossary.json`, `translation/locale.json`; `conversion/{announcement,kakao_notice,explainer,casual,kol,pr}.md`(**`x.md`는 읽지 않습니다** — 아래 참고); 타입별 few-shot 코퍼스(`few_shot_examples` 테이블, 역시 `conversion:x` 제외) | `output/variants/worksheets/`에 `batch-<타임스탬프>.md` 워크시트; `output/variants/pending.json` 갱신(이전 배치는 `output/archive/<YYYY-MM-DD>/`로 이동). **`x` 유형은 워크시트에도 `pending.json`에도 들어가지 않고 `variants` 테이블에 곧바로 저장됩니다** — 승인본을 그대로 변환본으로 쓰기 때문이며(`PrepareConversions`의 `PASSTHROUGH_TYPE`), 저장한 건수는 `saved n x variant(s) from the approved translation — no agent pass needed`로 찍힙니다. `--max-variants N`은 부풀린 **(항목, 유형) 쌍**을 N개로 자릅니다 — **`x`는 이 상한에 세지 않습니다**(에이전트 시간을 쓰지 않으므로) — `--limit`이 항목을 세는 것과 다른 단위이고, 잘린 쌍은 변환본 행이 없으니 다음 실행에서 다시 선택됩니다(스케줄러가 `claude -p` 10분 상한 때문에 쓰는 플래그) | 없음 |
...
| `pnpm convert:tick` | `HERALD_CONVERT_BATCH`(env, 기본 1). 스스로는 아무것도 읽지 않고 `pnpm convert:prepare --limit N --max-variants 4` → (에이전트가 할 일이 준비된 게 있을 때만 — `x`만 남은 fire는 여기서 멈춥니다) `claude -p` → `pnpm status`(전후 2회) → `pnpm format --only-missing`(**매번**, 준비된 게 없어도)을 하위 프로세스로 실행합니다 — 각 단계가 읽는 것은 그 단계의 행을 보세요 | 준비된 게 없으면 워크시트도 `pending.json`도 쓰지 않습니다. 있으면 `convert:prepare`/에이전트의 `convert:save`가 쓰는 것과 동일 — `output/variants/worksheets/`의 워크시트, `output/variants/pending.json`, `output/variants/variants.json`. 그리고 마지막 단계가 **아직 렌더링이 없는** (항목·유형·채널)만 골라 `output/formatted/renderings.json`에 씁니다 — **이미 있는 렌더링은 절대 건드리지 않고**(2차 검수에서 고친 글과 승인 상태가 그대로 남습니다), **1차 번역이 `posted`인 항목의 카드는 아예 만들지 않습니다**(이미 나간 항목이라 만들어 봐야 2차 검수 화면에 치울 수 없는 일감으로 남습니다). 에이전트가 넘겨받은 수보다 적게 저장하면 tick이 **실패**합니다 | `claude -p`(로컬 Claude Code CLI). 변환본은 `rendered`(2차 검수 대기)에서 멈추며 X·텔레그램·Typefully·Drive로는 아무것도 나가지 않습니다 |
| `pnpm format [--ids] [--types] [--channels] [--only-missing] [--refine]` | 변환본 원장 `variants`(`PgConversionStore` — **변환본** 상태 무관, 변환본 승인 게이트는 없습니다); 번역 원장 `translations`(`PgTranslationStore` — 1차 상태만, `posted`인 항목을 걸러내기 위해). `--only-missing`이면 기존 렌더링 키(`PgFormattingStore`)도 읽습니다 | 기본 모드: 렌더링 원장 `renderings`(`PgFormattingStore`)에 canonical 텍스트(`**볼드**`, `[텍스트](URL)`, 빈 줄 하나 = 문단 구분, 빈 줄 두 개 = 트윗 경계)로 직접 upsert — 입력의 `---` 한 줄도 경계로 인식하지만 저장되는 값은 항상 빈 줄 두 개입니다. **기본 모드는 덮어씁니다**: 고른 (항목·유형·채널)의 기존 글과 승인 상태가 사라지고 `rendered`로 되돌아갑니다(대시보드 `포맷 다시`와 같은 동작). `--only-missing`이면 아직 렌더링이 없는 짝만 만들고 기존 것은 그대로 둡니다 — 스케줄러(`convert:tick`)가 쓰는 모드이며, `--refine`과는 같이 못 씁니다. **1차 번역이 `posted`인 항목은 어느 모드에서도 만들지도 덮어쓰지도 않습니다**(이미 나간 항목은 끝난 항목입니다) — 건너뛴 개수는 요약 줄 아래에 `skipped n item(s) already posted`로 찍히고, 다시 만들어야 하면 1차에서 `되돌리기`부터 하세요. `--refine` 모드: `output/formatted/worksheets/`에 `batch-<타임스탬프>.md`(채널 제약 + 초안에 등장한 용어집 + 세그먼트별 길이 리포트 포함), `output/formatted/pending.json` 갱신(이전 배치는 `output/archive/<YYYY-MM-DD>/`로 이동) | 없음 |
| `pnpm format:save --id --type --channel --file` | `output/formatted/pending.json`(**진짜 파일**; 없으면 렌더링 원장 `PgFormattingStore`에서 폴백); `--file`(canonical 텍스트) | 렌더링 원장 `renderings`(`PgFormattingStore` — upsert, `refined: true`, canonical 텍스트 그대로 저장) | 없음 |
| `pnpm glossary [add --term --rule ...]` | `translation/glossary.json` | `add` 서브커맨드일 때만 `translation/glossary.json`(upsert) | 없음 |
| `pnpm glossary:mine [--notify]` | 원문 트윗 전체(`x_threads` 테이블 — 삭제된 스레드 포함); 번역 원장(`translations` — 발행 원문이 채워진 행의 초안↔발행 대조용); `lineage`(`stage="translated"` 행 — 정렬 이후 사람이 고친 부분을 뜨기 위해, `actor`가 없는 옛 행은 제외); `translation/glossary.json`(이미 결정된 용어 제외 — 비어 있으면 모든 고유명사가 후보가 되어 버리므로 실행 자체를 거부); `translation/glossary-dismissed.json`(사람이 이미 "아니오"라고 한 후보 — 없으면 빈 배열); `$OUTPUT_DIR/x/reference/items.json`+`runs.json`(0xMantleKR 참조 코퍼스와 그 수집 기간 — **없거나 28일 이상 지났으면 등급을 전부 B로 낮추고 그 사실을 표준 출력·알림에 함께 적습니다**. 이 명령은 수집하지 않습니다 — `collect:reference`는 수동 전용이고, 그 결과를 스케줄러 트리로 옮기는 것은 배포 4단계입니다). `--notify`면 `TELEGRAM_BOT_TOKEN`+`TELEGRAM_CHAT_ID_OPS`(env, 둘 다 있어야 함) | `$OUTPUT_DIR/glossary/candidates-<YYYY-MM-DD>.json` — 사람이 읽고 고쳐서 `pnpm glossary add`로 넣는 **검토 파일**(같은 날 다시 돌리면 덮어씁니다). 고유명사 후보는 **문장 중간에 한 번이라도 나온 것만** 올라오고, 줄머리에만 나와서 뺀 낱말이 몇 개인지는 검토 파일 머리말과 표준 출력에 함께 적습니다(2026-08-11 실측 170→90). 표준 출력에는 후보 목록과 **그 파일의 절대경로**. 후보가 나와도 종료 코드는 그대로 `0` | 없음. `--notify`이고 **후보가 하나라도 있을 때만** Telegram Bot API(`sendMessage`) 1회 — 등급·후보·근거 숫자와 검토 파일 절대경로만 보냅니다(글 본문은 안 보냄). 기각 목록만으로는 알림을 띄우지 않고, 후보가 없으면 코퍼스가 오래됐어도 조용합니다. 알림이 실패해도 종료 코드는 `0` |
| `pnpm config:init` | `translation/*.example.*`, `conversion/*.example.*` | 실제 파일이 아직 없는 것만 생성 — `*.example.*`가 있는 이름 전부라서 `few-shot*.json`의 빈 스켈레톤도 함께 만들어집니다(런타임에 읽히지는 않습니다 — §1). 이미 있으면 절대 덮어쓰지 않음 | 없음 |
...
| `pnpm x:reconcile [--since] [--handle] [--yes]` | `local` 모드면 스킵. `REFERENCE_X_HANDLE`(env, 기본 `0xMantleKR`) 또는 `--handle`; `TWITTERAPI_IO_KEY`(env, 계정의 live 트윗을 스레드 단위로 재구성); 승인된 `x` 렌더링을 위한 `output/formatted/renderings.json`; 중복 방지를 위한 `output/publish/deliveries.json`(딜리버리 키); Google Sheet `history` 탭(A열 `itemId`·D열 `postId` — 이미 기록된 게시물 식별용, 탭이 아직 없으면 빈 것으로 취급); 번역 원장 `output/translations/translations.json`(`source: "x"`인 행만) | `--yes`일 때만: `output/publish/deliveries.json`에 확정된 항목의 `x-post` 아웃렛 `sent` 행 추가; Google Sheet `history` 탭에 파이프라인 밖 글 + 손-게시로 게시됨 처리된 글을 각각 한 행씩 upsert; 번역 원장 `output/translations/translations.json` — 손-게시로 확인된 번역의 `status`를 `posted`로, `postedUrl`/`postedAt`을 채움(한 번 채워지면 절대 덮어쓰지 않음), **그렇게 게시됨으로 남은 항목 중 이 실행의 `--since` 창에 아직 걸리는 것은 실제 발행 원문도 함께 채움(역시 빈 값만 채우고 절대 덮어쓰지 않음)**; 한 번의 실행에서 3건 이상을 게시됨으로 못박으면 텔레그램 알림(`TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID_OPS`, 둘 중 하나라도 없으면 조용히 건너뜀). `--yes` 없으면 위 어느 것도 쓰지 않고 계획(확정/후보/외부/게시됨/캡처할 원문 건수)만 출력 | twitterapi.io API(계정 타임라인 재조회), Google Sheets API(`history` 탭 조회+기록) |
| `pnpm x:link --item <itemId> --post <id\|url> [--handle] [--yes]` | `local` 모드면 스킵. `REFERENCE_X_HANDLE`(env, 기본 `0xMantleKR`) 또는 `--handle`; `TWITTERAPI_IO_KEY`(env, **ID로 직접 조회** — 계정 목록 조회를 쓰지 않는 것이 이 명령의 요점); Google Sheet `history` 탭(D열 `postId` — 같은 게시물에 두 번째 행을 만들지 않기 위한 가드); 번역 원장(`source: "x"`인 행만) | `--yes`일 때만: 지정한 번역의 `status`를 `posted`로, `postedUrl`/`postedAt`을 채우고, Google Sheet `history` 탭에 한 행 upsert, 실제 발행 원문을 `published_text`에 채움(빈 값만, 절대 덮어쓰지 않음). `--yes` 없으면 아무것도 쓰지 않고 대상·시각·유사도만 출력. **승인하지 않고, 발송하지 않고, X에 아무것도 쓰지 않으며, 수집 워터마크도 건드리지 않습니다.** 유사도가 낮으면 경고만 하고 막지는 않습니다 — 이 명령은 매칭기가 못 본 글을 사람이 지정하는 자리라, 기계 점수가 사람을 막으면 존재 이유가 사라집니다 | twitterapi.io API(게시물 ID 조회 + 스레드 문맥), Google Sheets API(`history` 탭 조회+기록) |
| `pnpm x:video-backfill [--yes]` | `TWITTERAPI_IO_KEY`(env, **ID로 직접 조회** — `x:link`과 같은 이유로 계정 타임라인 조회를 쓰지 않습니다); 수집된 스레드 전부(`x_threads`, **상태 무관** — 삭제된 스레드의 본문도 검수 화면이 읽으므로 함께 봅니다) 중 `videoUrl`이 비어 있는 `video`/`animated_gif` 미디어. 재수집(`pnpm collect --since 70d`)으로 대부분은 채워지지만 계정 타임라인에 더 이상 잡히지 않는 글에는 어떤 `--since`로도 닿지 못하며, 그런 글은 ID 조회만이 유일한 경로입니다 | `--yes`일 때만: **실제로 채워진 스레드만** `x_threads`에 upsert — 미디어의 `videoUrl` 한 칸만 더해지고 본문·아티클 블록·지표·`status`/`firstSeenAt`/`deletedAt`·스레드 순서는 저장된 값 그대로입니다(짝은 배열 위치가 아니라 썸네일 `url`로 맞추므로 영상이 둘인 글도 뒤바뀌지 않습니다). mp4가 없거나 API가 그 id를 돌려주지 않은 글은 **채운 것으로 표시하지 않고 후보로 그대로 남으며**(다음 실행이 다시 봅니다) id를 한 줄씩 출력합니다. `--yes` 없으면 아무것도 쓰지 않고 **`--yes`와 똑같은 계획**(후보 개수, 걸린 스레드 수, 채울 수 있는 개수, 못 채운 id)만 출력 | twitterapi.io API(`GET /twitter/tweets`, 50개씩 배치 — 채울 후보가 하나도 없으면 호출 자체를 하지 않음) |
| `pnpm text:video-backfill [--yes]` | 수집된 스레드 전부(`x_threads`, **상태 무관** — 삭제된 스레드의 번역도 검수 화면이 읽으므로 함께 봅니다. 여기서는 **읽기만** 합니다); 번역 원장의 `source_text`·`korean_text`와 채널 렌더링의 `text` 중 주소 없는 `[영상]` 마커가 남아 있는 것. 짝은 저장된 미디어 배열이 아니라 **마커를 실제로 써 넣은 경로**(`flattenXThreads` → `extractMedia`)로 다시 구해 맞춥니다 — 중첩 답글과 아티클 본문은 마커를 만들지 않으므로, 미디어 배열을 그대로 세면 그 뒤의 짝이 통째로 한 칸씩 밀립니다 | `--yes`일 때만: **실제로 달라지는 행만** upsert — `translations.source_text`, `translations.korean_text`, `renderings.text` **세 칸이 전부**입니다. **`published_text`는 절대 쓰지 않습니다**(사람이 실제로 올린 원문의 기록이라 이 명령이 손댈 자리가 아닙니다). 마커 개수와 그 글의 영상 개수가 1:1로 맞지 않거나, 스레드 쪽 영상에 아직 mp4가 없거나, 그 항목의 수집 스레드 자체가 없으면 **그 본문은 통째로 건너뛰고 이유와 함께 한 줄씩 출력합니다** — 절반만 채운 본문은 완성된 것처럼 보이면서 엉뚱한 영상을 물고 있을 수 있어, 안 건드린 것보다 나쁩니다. `--yes` 없으면 아무것도 쓰지 않고 **`--yes`와 똑같은 계획**만 출력하며, 계획은 번역과 렌더링을 **따로** 적습니다 — 번역은 검수 화면 표시만 바뀌고(이미 나간 글이라 재발송 없음), 렌더링은 **다음 발송이 실제로 첨부하는 것**이 바뀝니다(`send:channels`는 주소가 실린 마커만 업로드합니다) | 없음 — DB만 읽고 씁니다(twitterapi 호출 없음, `TWITTERAPI_IO_KEY`도 필요 없음) |
...
| `pnpm kol-telegram:record [--month <YYYY-MM>]` | `local` 모드면 스킵. `GSHEET_ID`(env) 워크북의 `'KOL list'` 탭(`active`인 행만 — kolId·(`Social media link` 컬럼에서 읽는) 텔레그램 핸들·sheetLabel·pricePerPost·active — 2026-08-19에 `kol-map`에서 이관, [`kol-map-seed.md`](kol-map-seed.md) 참고); 각 활성 채널의 공개 프리뷰 페이지 `https://t.me/s/<handle>`를 `--month` 구간에 걸릴 때까지 페이지네이션(공식 API 아님, 별도 키 불필요); 매칭 후보로 `output/formatted/renderings.json`(`status: "approved"`이고 채널이 `telegram`인 행만, `FormattingStore.loadAll()` 경유). 기본 대상은 이번 달 | 같은 워크북의 `kol-telegram-posts` 탭(없으면 생성)에 `deliverableLink`(텔레그램 permalink) 키로 upsert — 신규 행은 전체 컬럼을 채움. 기존 행은 `views`/`engagements`/`reactionsDetail`/`fetchedAt` 4개를 갱신하고(단 `confirmed: "reject"` 행은 아예 손대지 않으므로 **그 행의 측정값도 함께 멈춤**), `itemId`/`matchScore`/`topic`/`pricePerPost`는 **그 칸이 아직 빈 경우에만** 채움(한 번 채워지면 다음 실행이 덮어쓰지 않음 — 이 블랭크 전용 규칙 덕분에 승인된 문구가 없어 빈 채로 만들어진 7월 행도, 문구가 생긴 뒤 같은 달을 다시 돌리면 소급 채워질 수 있음. 단 지난 달 재실행은 페이지 상한에 걸려 그 달에 닿지 못할 수 있으니 요약의 `channel(s) truncated`를 확인); **갱신 시 실제로 값이 달라진 칸만 씀** — 측정값만 바뀐 보통의 경우 `E:G`와 `L`만 쓰므로 `pricePerPost`(K)와 `confirmed`(M)는 기계가 쓰는 범위에 들어가지 않고, 바뀐 칸이 없으면 아무것도 쓰지 않음; `confirmed`와 이미 채워진 `topic`은 절대 안 덮어씀; `confirmed: "reject"` 행은 다시 제안하지 않음. 신규 행은 한 번의 append 호출로 모아서 씀(행마다 한 번씩 쓰면 Sheets의 분당 60회 쓰기 한도를 넘김). **`kol-map`/`KOL list`/계약 리스트/월별(`Jul.`/`Aug.`/`Sep.`) 탭은 전혀 쓰지 않음**(그중 `KOL list`만 읽기 전용으로 읽음 — `kol-map`은 은퇴한 탭이라 아예 건드리지 않음); `confirmed`는 사람이 채우는 유일한 컬럼이고, 확정된 행을 월별 탭으로 옮기는 것도 사람의 몫 | Google Sheets API(`KOL list` 조회, `kol-telegram-posts` 갱신). 텔레그램 쪽은 공개 웹페이지 요청뿐이라 봇 토큰이나 별도 API 키가 필요 없음 |
| `pnpm kol:quarter [--quarter <YYYY-Qn>]` | `local` 모드면 스킵. `GSHEET_ID`(env) 워크북의 `'KOL list'` 탭(로스터, `kol-telegram:record`와 같은 방식으로 읽음); ` Q<n> KOL 계약 리스트` 탭의 `Deliverables` 컬럼(그 달 필요 게시 건수 — 숫자/`unlimited`/읽을 수 없음 세 갈래로 파싱); `kol-telegram-posts` 탭(분기의 달마다 `kol-telegram:record`를 내부적으로 다시 돌려 최신화한 뒤 그 결과를 읽음). `--quarter` 생략 시 오늘이 속한 분기 | 분기의 각 달(`Jul.`/`Aug.`/`Sep.` 등) 로그 영역에 **값 일곱 열만** upsert — `KOL`/`Social`(또는 `Social media platform`)/`Posting date`/`Deliverable Link`/`Content Views`/`Engagements`/`Price per posting`뿐 — 로그의 `Topic` 컬럼(E)은 사람 전용이라 이 명령이 전혀 건드리지 않습니다(`logCells`가 명시적으로 제외, `tests/app/projectMonthlyLog.test.ts`가 고정). 빈 칸일 때만 채우는 `topic`은 이 로그가 아니라 `kol-telegram-posts` 탭 쪽 컬럼입니다 — 위 `kol-telegram:record` 행 참고. `Posting date`는 Sheets 날짜 시리얼(정수)로 씀. 로그 헤더 행은 찾아서 쓸 뿐 위치·스펠링을 가정하지 않음. 요약 블록(1-8행)·`Total Cost`·세 수식 컬럼(`Engagement Rate`/`Cost per impression`/`Duplicated?`)·`followers`·`Organic`은 전혀 안 씀 — 그 칸들은 사람 또는 시트 자체의 수식 몫. 계약 대비 부족분, 계약 이름 중 로스터와 매칭 안 된 것, 파싱 못한 `Deliverables`는 표준 출력에만 보고하고 어떤 셀에도 쓰지 않음 | Google Sheets API(로스터/계약 탭 읽기, `kol-telegram-posts` 갱신, 월별 로그 upsert), 각 활성 채널의 공개 텔레그램 프리뷰 페이지(`kol-telegram:record`와 같은 스윕을 분기의 세 달 모두에 반복) |
| `pnpm kol-roster-migrate [--yes]` | `local` 모드면 스킵. `GSHEET_ID`(env) 워크북의 `'kol-map'` 탭(전체 13행, `active` 무관하게 다 읽음)과 `'KOL list'` 탭(헤더 + 기존 `Social media link` 컬럼) | `--yes` 없으면 아무것도 안 쓰고 계획(배치될 행·배치 못한 행)만 출력. `--yes`면 `kol-map`의 각 행을 `Social media link`에서 뽑은 텔레그램 핸들로 짝지어지는 `KOL list` 행에 `kolId`/`sheetLabel`/`pricePerPost`/`active`(없으면 새로 만든 네 컬럼) upsert + 그 행의 `Social media link`가 빈 칸이면 핸들도 채움, 그다음 `'kol-map'!A1`에 `(은퇴 — KOL list로 이관)` 기록. 핸들로 짝지어지지 않는 행은 **이름으로 추측하지 않고** 그대로 보고만 되며 아무 데도 안 씀(자세한 이유는 [`kol-map-seed.md`](kol-map-seed.md) 참고) | Google Sheets API(`kol-map`/`KOL list` 조회 + 갱신) |
| `pnpm sheet:init` | `local` 모드면 스킵. 로컬 파일 없음 | 로컬 파일 없음 — 생성된 스프레드시트 id를 콘솔에 출력 | Google Sheets API(스프레드시트 + `targets`/`history` 탭 생성) |
| `pnpm doctor [--live]` | 모든 env 설정 로더; `translation/glossary.json`, `translation/style-guide.md`, `translation/locale.json`, 그리고 변환 유형마다 `conversion/<type>.md`의 존재 여부와 각각이 `*.example.*` 스켈레톤 그대로인지. **배포 체크아웃이 식별되면**(systemd에게 물은 `herald-watch.service`의 `WorkingDirectory`, 또는 `HERALD_DEPLOY_DIR`) 그쪽 `translation/`·`conversion/`·`keys/`의 파일도 읽어 이 트리와 대조합니다 — 목록은 `deploy:freeze`와 **같은 코드**(`steeringFilesIn`)로 뽑으므로 `few-shot*.json`은 이쪽 목록에서 빠지고 배포 트리 쪽에서는 빠지지 않습니다(§ 그래서 예전 배포가 얼려 둔 사본이 한 번 보이고 다음 배포에 쓸려 나갑니다). 비교는 sha-256이고 리포트에 나가는 건 **파일 이름뿐**입니다. 데이터베이스 쪽은 두 줄 — 연결·스키마 확인(`deliveries` 한 행 조회)과, `few_shot_examples`에서 `item_id`가 빈 행을 **스코프별로 센 개수**(`Few-shot corpus keys`, [`setup/steering.md`](setup/steering.md#3-1-few-shot-corpus-keys--item_id-없는-예시) §3-1 — 예시 본문은 읽지 않습니다) | 없음 | `--live`일 때만: Google OAuth tokeninfo, Google Drive/Sheets 파일 메타데이터 조회, Lark 인증 + 채팅 목록 조회. `--live` 없이도 `systemctl --user show herald-watch.service`는 부릅니다(로컬 프로세스, 네트워크 아님) |
| `pnpm status` | **데이터베이스만** — `createStores(db)`로 수집 스레드·번역·변환본·렌더링·동기화 원장을 조회합니다(`src/cli/status.ts`). 로컬 파일은 하나도 읽지 않으므로 `DATABASE_URL`이 없거나 스키마가 아직 없으면 실패합니다. 스케줄러의 번역 floor는 `systemctl show`와 DB의 마지막 기록에서 읽습니다 | 없음 | 없음(`systemctl show`는 로컬 프로세스) |
...
- `output/formatted/overrides.json` — 방별로 따로 검수·승인한 글(override). 그룹 렌더링과는 별개로
  ...
  자체는 재생성할 수 없습니다.
- 실제 스티어링 파일: `translation/glossary.json`, `translation/glossary-dismissed.json`,
  `translation/style-guide.md`, `translation/locale.json`, `translation/tm.json`,
  `conversion/{x,announcement,kakao_notice,explainer,casual,kol,pr}.md`,
  `conversion/checklist.{x,announcement}.md`. **`few-shot*.json`은 이 목록에 없습니다** — 코퍼스는
  `few_shot_examples` 테이블이고, 그 파일들은 `db:export` 산출물입니다(데이터베이스 쪽은 아래
  `state:push` 상자가 챙깁니다)
...
> 차이가 납니다.
>
> **few-shot 코퍼스만 예외로, 스코프 단위 교체입니다.** 위 일곱 개는 자연키로 식별되고 행 순서에
> 의미가 없지만, `few_shot_examples`는 `ordinal` 순서가 곧 프롬프트에 들어가는 내용이고 스냅샷이
> 그 스코프의 코퍼스 **전체**를 담습니다. 그래서 스냅샷에 있는 스코프는 기존 행을 지운 뒤 스냅샷
> 순서대로 다시 넣습니다 — 미리보기가 `덮어씀`이라고 말한 그대로입니다. 그러지 않으면 이미 예시가
> 있는 데이터베이스에 복구할 때 집합은 맞고 **순서만 조용히 바뀝니다**(대상에 `{C}`가 있는 상태로
> `[A, B, C]`를 복구하면 C, A, B가 됩니다). 스냅샷에 없는 스코프는 `유지`로 두고 건드리지 않습니다.
>
