...
**저장 모드를 실제로 읽어서 검사하는 명령은 소수뿐입니다** — 아래 표의 아홉 개 CLI(`drive:init`/
`sheet:init`/`targets:list`/`history:record`/`impressions:record`/`metrics:record`/
`kol-telegram:record`/`x:reconcile`/`x:link`), `pnpm doctor`, `pnpm drive:publish`, `pnpm serve`가
전부입니다.
...
| `drive:init`, `sheet:init`, `targets:list`, `history:record`, `impressions:record`, `metrics:record`, `kol-telegram:record`, `x:reconcile`, `x:link` (아홉 개) | `"<command>: local mode — skipped (set HERALD_STORAGE_MODE=cloud to enable)"`을 출력하고 종료 코드 `0` | 정상 실행 |
