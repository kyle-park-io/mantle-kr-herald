...
| `pnpm doctor [--live]` | 모든 env 설정 로더; `translation/glossary.json`, `translation/style-guide.md`, `translation/locale.json`, 그리고 변환 유형마다 `conversion/<type>.md`의 존재 여부와 각각이 `*.example.*` 스켈레톤 그대로인지. **배포 체크아웃이 식별되면**(systemd에게 물은 `herald-watch.service`의 `WorkingDirectory`, 또는 `HERALD_DEPLOY_DIR`) 그쪽 `translation/`·`conversion/`·`keys/`의 파일도 읽어 이 트리와 대조합니다 — 목록은 `deploy:freeze`와 **같은 코드**(`steeringFilesIn`)로 뽑으므로 `few-shot*.json`은 이쪽 목록에서 빠지고 배포 트리 쪽에서는 빠지지 않습니다(§ 그래서 예전 배포가 얼려 둔 사본이 한 번 보이고 다음 배포에 쓸려 나갑니다). 비교는 sha-256이고 리포트에 나가는 건 **파일 이름뿐**입니다 | 없음 | `--live`일 때만: Google OAuth tokeninfo, Google Drive/Sheets 파일 메타데이터 조회, Lark 인증 + 채팅 목록 조회. `--live` 없이도 `systemctl --user show herald-watch.service`는 부릅니다(로컬 프로세스, 네트워크 아님) |
