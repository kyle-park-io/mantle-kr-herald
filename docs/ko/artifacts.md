...
- **워터마크 증분** (정기 스케줄러가 쓰는 방식) — 플래그 없는 `pnpm collect`. 실제 자동화는
  ...
  `collect`를 아무 인자 없이 실행하도록 되어 있습니다 — 의도된 설계입니다. 재수집이 최소이고,
  워터마크가 최신으로 전진한 뒤 늦게 인덱싱된 트윗은 영구히 건너뛸 수 있고 오버랩 안전망이
  없다는 게 대가지만, 정기 스케줄러가 이 방식을 필요로 하는 진짜 이유는 재수집량이 아니라
  워터마크 자체입니다: 신규 스레드 0건이면 그 tick이 에이전트를 아예 부르지 않고 끝나는
  게이트가 워터마크 전진에 의존합니다.
