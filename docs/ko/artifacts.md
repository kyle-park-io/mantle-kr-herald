...
| `pnpm glossary:mine [--notify]` | 원문 트윗 전체(`x_threads` 테이블 — 삭제된 스레드 포함); 번역 원장(`translations` — 발행 원문이 채워진 행의 초안↔발행 대조용); `translation/glossary.json`(이미 결정된 용어 제외 — 비어 있으면 모든 고유명사가 후보가 되어 버리므로 실행 자체를 거부); `translation/glossary-dismissed.json`(사람이 이미 "아니오"라고 한 후보 — 없으면 빈 배열); `$OUTPUT_DIR/x/reference/items.json`+`runs.json`(0xMantleKR 참조 코퍼스와 그 수집 기간 — **없거나 28일 이상 지났으면 등급을 전부 B로 낮추고 그 사실을 표준 출력·알림에 함께 적습니다**. 이 명령은 수집하지 않습니다 — `collect:reference`는 수동 전용입니다). `--notify`면 `TELEGRAM_BOT_TOKEN`+`TELEGRAM_CHAT_ID_OPS`(env, 둘 다 있어야 함) | `$OUTPUT_DIR/glossary/candidates-<YYYY-MM-DD>.json` — 사람이 읽고 고쳐서 `pnpm glossary add`로 넣는 **검토 파일**(같은 날 다시 돌리면 덮어씁니다). 표준 출력에는 후보 목록과 **그 파일의 절대경로**. 후보가 나와도 종료 코드는 그대로 `0` | 없음. `--notify`이고 **후보가 하나라도 있을 때만** Telegram Bot API(`sendMessage`) 1회 — 등급·후보·근거 숫자와 검토 파일 절대경로만 보냅니다(글 본문은 안 보냄). 기각 목록만으로는 알림을 띄우지 않고, 후보가 없으면 코퍼스가 오래됐어도 조용합니다. 알림이 실패해도 종료 코드는 `0` |
