...
| **F. 콘텐츠 가공** | 승인된 번역을 §5 항목 변환(타입별 X/공지/카톡 공지/해설/소통/KOL/PR)과 §6 채널 포맷(코드 변환 + 선택적 에이전트 다듬기) 두 단계로 채널용 게시물로 가공 | `pnpm convert:prepare`, `pnpm convert:save`, `pnpm format`, `pnpm format:save` | — |
