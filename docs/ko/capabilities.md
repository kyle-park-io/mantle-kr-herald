...
| **P. KOL 분기 딜리버러블 트래킹** | `2026 Q3 KR Work Sheet`의 월별 탭(`Jul.`/`Aug.`/`Sep.`)은 두 표로 되어 있습니다 — 위쪽 요약 블록(팀이 이미 짜 둔 `COUNTIF`/`SUMIF` 수식이 아래 로그를 집계)과 그 아래 게시물 한 줄당 한 행인 로그. 이 기능은 그 **로그 쪽만** 채웁니다: 분기의 매달마다 `kol-telegram:record`를 내부적으로 다시 돌려 `kol-telegram-posts`를 최신화한 뒤, 그 행들을 로그 영역의 **값 일곱 열**(`KOL`/`Social`(또는 `Social media platform`)/`Posting date`/`Deliverable Link`/`Content Views`/`Engagements`/`Price per posting`)과 빈 칸일 때만 `Topic`에 투영합니다. **요약 블록과 `followers`는 절대 건드리지 않습니다** — `Posts`/`Views`/`Engagement`/`Total Cost`와 세 비율 컬럼(`Engagement Rate`/`Cost per impression`/`Duplicated?`)은 전부 로그 위의 기존 수식이 계산하고, 이 기능은 그 수식이 읽는 값만 채울 뿐 수식 자체는 한 줄도 쓰지 않습니다(값 API로 `=`로 시작하는 문자열을 쓰면 수식이 아니라 문자 그대로 저장되기 때문 — `GoogleSheetClient`가 세 쓰기 메서드 모두 `valueInputOption=RAW`라 애초에 못 씁니다). 그래서 새 로그 행에 비율이 나오려면, 그 세 수식과 `Posting date`의 날짜 서식을 사람이 로그 맨 아래까지 한 번 채워 둬야 합니다(마이그레이션 3단계, 시트를 직접 채우는 유일한 수작업). 계약 대비 필요 게시 건수는 ` Q<n> KOL 계약 리스트`의 `Deliverables`에서 읽되(`Monthly (10)`처럼 숫자, `TG (8)`, 또는 `Monthly (unlimited)`) 이 실행의 보고에만 쓰이고 어떤 셀에도 적히지 않습니다 — 계약 이름은 대소문자·공백을 지우고 로스터의 `sheetLabel`과 맞추며, 맞는 로스터가 없는 이름은 부족분이 아니라 **매칭 안 됨**으로 따로 보고합니다 | `pnpm kol:quarter [--quarter YYYY-Qn]` | [`schedulers.md`](schedulers.md) |
