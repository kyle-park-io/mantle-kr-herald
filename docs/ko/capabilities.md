...
| **F. 콘텐츠 가공** | 승인된 번역을 §5 항목 변환(타입별 X/공지/카톡 공지/해설/소통/KOL/PR — **`X`만은 재작성하지 않고 승인본을 그대로 씁니다**: 원문이 X 게시물이므로 그 번역이 곧 우리 X 게시물이고, 에이전트도 few-shot도 거치지 않습니다)과 §6 채널 포맷(코드 변환 + 선택적 에이전트 다듬기) 두 단계로 채널용 게시물로 가공 | `pnpm convert:prepare`, `pnpm convert:save`, `pnpm format`, `pnpm format:save` | — |
...
| **L. 설정 백업/공유** | git에 추적되지 않는 스티어링 설정(`translation/` + `conversion/`, `*.example.*`와 `few-shot*.json` 제외 14개 파일)을 Google Drive에 타임스탬프 스냅샷(`steering-config-<시각>.json`)으로 백업하고, 팀원이 최신 스냅샷을 내려받아 복원 — 단일 관리자가 push(백업), 팀원은 pull(복원)만 하는 모델. `pull`은 덮어쓰기 전에 현재 로컬 설정을 `output/archive/`에 먼저 백업 | `pnpm config:push`, `pnpm config:pull [--dry-run]` | — |
| **M. 운영 상태 백업/복구** | 데이터베이스에서 **다시 만들 수 없는** 일곱 개 — 사람이 검수한 글(`translations/translations.json`, `variants/variants.json` — 다시 돌리면 *어떤* 결과는 나오지만 그 결과는 아니고 검수를 다시 해야 합니다), 채널별 최종 렌더링과 그 **2차 검수 승인 상태**(`formatted/renderings.json` — 렌더링 텍스트 자체는 `format`이 변환본에서 다시 만들지만, 사람이 승인했다는 사실이나 손으로 다듬은 내용은 재생성되지 않습니다), 방별 포크(`formatted/overrides.json`), 발송 원장(`publish/deliveries.json`, `publish/x-article.json` — 예전 형식 `publish/channels.json`은 `pnpm db:import`가 그 행을 이미 `deliveries.json` 쪽으로 옮겨 놓았으므로 더는 따로 스냅샷하지 않습니다), 동기화 원장(`publish/state.json`) — 을 Google Drive에 타임스탬프 스냅샷(`operational-state-<시각>.json`)으로 백업하고 되살림. L과 달리 **공유가 아니라 이 기기 한 대의 복구**이므로, `pull`은 `--yes` 없이는 미리보기만 하고 파일마다 **현재 행 수와 스냅샷 행 수를 나란히** 보여주며, 쓰기 전에 데이터베이스의 현재 내용을 `output/archive/state-<시각>/`에 백업하고, 파싱·백업이 실패하면 아무것도 쓰지 않고 중단. 스냅샷의 각 행을 데이터베이스에 **가져올(upsert)** 뿐 대체하지 않으므로, 스냅샷에 없는 기존 행은 지워지지 않고 그대로 남음(단 few-shot 코퍼스는 예외 — `ordinal` 순서가 곧 프롬프트 내용이고 스냅샷이 스코프의 코퍼스 전체를 담으므로 **스코프 단위로 교체**합니다) | `pnpm state:push`, `pnpm state:pull [--yes]` | — |
| **N. 텔레그램 KOL 딜리버러블 동기화** | 사람이 관리하는 `KOL list` 탭(활성 행만 — 2026-08-19에 `kol-map`에서 이관, [`kol-map-seed.md`](kol-map-seed.md) 참고)을 읽어 각 텔레그램 KOL의 공개 채널을 그 달 구간으로 훑고, 맨틀 언급 후보 게시물을 찾아 승인된 텔레그램 렌더링과 매칭(제안)한 뒤 기계 전용 `kol-telegram-posts` 탭에 게시물 단위로 upsert | `pnpm kol-telegram:record [--month YYYY-MM]` | — |
...
| **P. KOL 분기 딜리버러블 트래킹** | `2026 Q3 KR Work Sheet`의 월별 탭(`Jul.`/`Aug.`/`Sep.`)은 두 표로 되어 있습니다 — 위쪽 요약 블록(팀이 이미 짜 둔 `COUNTIF`/`SUMIF` 수식이 아래 로그를 집계)과 그 아래 게시물 한 줄당 한 행인 로그. 이 기능은 그 **로그 쪽만** 채웁니다: 분기의 매달마다 `kol-telegram:record`를 내부적으로 다시 돌려 `kol-telegram-posts`를 최신화한 뒤, 그 행들을 로그 영역의 **값 일곱 열**(`KOL`/`Social`(또는 `Social media platform`)/`Posting date`/`Deliverable Link`/`Content Views`/`Engagements`/`Price per posting`)에만 투영합니다 — 로그의 `Topic` 컬럼은 사람 전용이라 전혀 건드리지 않습니다(`logCells`가 명시적으로 제외합니다). 빈 칸일 때만 채우는 `topic`은 이 로그가 아니라 `kol-telegram-posts` 탭 쪽 컬럼입니다(§9 참고). **요약 블록과 `followers`는 절대 건드리지 않습니다** — `Posts`/`Views`/`Engagement`/`Total Cost`와 세 비율 컬럼(`Engagement Rate`/`Cost per impression`/`Duplicated?`)은 전부 로그 위의 기존 수식이 계산하고, 이 기능은 그 수식이 읽는 값만 채울 뿐 수식 자체는 한 줄도 쓰지 않습니다(값 API로 `=`로 시작하는 문자열을 쓰면 수식이 아니라 문자 그대로 저장되기 때문 — `GoogleSheetClient`가 세 쓰기 메서드 모두 `valueInputOption=RAW`라 애초에 못 씁니다). 그래서 새 로그 행에 비율이 나오려면, 그 세 수식과 `Posting date`의 날짜 서식을 사람이 로그 맨 아래까지 한 번 채워 둬야 합니다(스펙의 마이그레이션 **2단계**, 시트를 직접 채우는 유일한 수작업). 사람이 `kol-telegram-posts`의 `confirmed`에 `reject`를 적은 게시물은 로그에도, 계약 집계에도 들어가지 않습니다(그게 사람의 취소 수단입니다). 각 행의 `Price per posting`은 그 게시물이 기록될 당시의 `pricePerPost`를 그대로 쓰고, 그 값이 비어 있을 때만 로스터의 단가로 채웁니다 — 로스터 단가를 나중에 고쳐도 이미 정산한 달의 `Total Cost`가 움직이지 않습니다. 새 행은 요약의 `SUMIF`가 닿는 **1963행까지만** 만들고, 다 차면 남은 게시물을 보고한 뒤 0이 아닌 코드로 끝냅니다(덮어쓰지 않습니다). 로스터가 비었거나 훑은 채널이 하나도 없어도 마찬가지로 실패로 끝납니다 — 매주 조용히 성공하는 게 이 기능의 가장 위험한 고장 방식이라서입니다. 계약 대비 필요 게시 건수는 ` Q<n> KOL 계약 리스트`의 `Deliverables`에서 읽되(`Monthly (10)`처럼 숫자, `TG (8)`, 또는 `Monthly (unlimited)`) 이 실행의 보고에만 쓰이고 어떤 셀에도 적히지 않습니다 — 계약 이름은 대소문자·공백을 지우고 로스터의 `sheetLabel`과 맞추며, 맞는 로스터가 없는 이름은 부족분이 아니라 **매칭 안 됨**으로 따로 보고합니다 | `pnpm kol:quarter [--quarter YYYY-Qn]` | [`schedulers.md`](schedulers.md) |
...
- **승격** — `pnpm tm:promote`는 사람이 승인한 페어만 `translation/tm.json`에 씁니다. 자동으로
  승격되는 페어는 없습니다.
- **번역 프롬프트에 반영** — `pnpm translate:prepare`는 기존의 큐레이션된 few-shot에 더해, 번역
  배치와 같은 언어 앵커가 겹치는 TM 페어 중 가장 관련도 높은 것들을 최대 6개까지 함께 워크시트에
  넣습니다(`MAX_TM_FEW_SHOTS`, `src/app/PrepareTranslations.ts`).
- **바뀐 것과 안 바뀐 것 — 여기서 "8"이 두 번 나오므로 주의하세요.** 큐레이션 few-shot을 고르는
  규칙은 **그대로**입니다: 지금도 가장 최근 8개입니다(`fewShots.slice(-MAX_FEW_SHOTS)`,
  `MAX_FEW_SHOTS = 8`). 대체된 것은 **프롬프트에 실리는 예시가 그 최근 8개뿐이던 규칙**입니다 —
  이제 그 8개 위에 관련도로 고른 TM 페어가 더 얹힙니다. 즉 "최근 8개"가 없어진 게 아니라, 그것만
  있던 상태가 없어진 것입니다.
...
**발행본 대조(선택)** — `pnpm translate:check [--status] [--since] [--published] [--notify]`은 초안이
...
드리프트가 나와도 종료 코드를 실패로 바꾸지 않습니다. `--notify`를 주면 **오버라이드가 잡혔을
때만** ops 방으로 한 통 알립니다(용어 · 결정 표현 · 몇 개 항목에서 뒤집혔는지; 글 본문은 안
보냄) — 정기 실행용 스위치입니다. 드리프트는 알림을 띄우지 않습니다: 사람이 하나씩 읽고
...
1. **용어집에 없는데 원문에서 반복되는 고유명사** — 수집된 영어 트윗 전체에서 대문자로 시작하는
   1~3어절 덩어리를 세어 2회 이상이고, **그중 최소 한 번은 문장 중간에 나오는 것만.** 위치가
   판별 기준입니다: 줄머리·문장머리의 대문자는 이름이라서가 아니라 자리 때문에 대문자입니다
   (`Together`, `Tomorrow`, `Weekly`, `Winners`…). 2026-08-11 첫 프로덕션 실행의 후보 170건 중
   110건이 정확히 그런 낱말이었고, 이 규칙이 90건으로 줄였습니다. 불용어 목록을 늘리는 방식은
   쓰지 않습니다 — 그 목록은 끝이 없고 늘 한 주씩 늦습니다.
   - `→`·`•` 불릿, `12)` 번호, `---` 구분선, `1️⃣` 같은 키캡 이모지는 전부 "줄머리 장식"으로 보고
     그 뒤를 여전히 줄머리로 셉니다. 한 줄 안의 두 번째 문장(`… 끝났습니다. Winners …`)도
     문장머리입니다. 반대로 `:`이나 em dash 뒤는 문장 중간으로 셉니다.
   - **한 번도 문장 중간에 안 나온 이름은 버립니다.** 525트윗 내내 줄머리에만 있던 말은 기계가
     보통 낱말과 구분할 방법이 없습니다(실측 희생자: `AGNI`, `Liftoff`, `RWAlly`). 영구 손실은
     아닙니다 — 계정이 그 말을 문장 안에 쓰는 순간 다음 주 후보로 올라옵니다. 몇 개를 뺐는지는
     검토 파일 머리말과 표준 출력에 함께 적습니다.
2. **사람이 고친 낱말** — 두 개의 수정 피드에서 같은 문장인데 낱말만 다른 자리를 찾습니다. 게시된 글과의 차이(초안↔발행본), 그리고 1차 검수에서 사람이 직접 고친 차이(`lineage` 기록에서 마지막 에이전트 버전 이후). 같은 치환이 두 피드 모두에 나타나면 양쪽 출처를 담은 후보 하나로 병합하고, 검토 파일에서 각 후보가 어느 피드에서 왔는지 표시합니다. **빈도 문턱이 없습니다**: 2026-08-11 실측에서 가장 값진 발견(`낸슨 → 난센`)은 딱 한 번 있었고, 3회 이상을 요구한 초기 시도는 그걸 통째로 놓쳤습니다.
...
**그 코퍼스는 배포가 실어 나릅니다.** `collect:reference`는 손으로 돌리는 명령이라 개발
체크아웃의 `<repo>/output/x/reference/`에 씁니다. 스케줄러는 `HERALD_OUTPUT_DIR=~/.herald/output`
아래를 읽으므로, 2026-08-11 첫 실제 발화에서는 대조할 코퍼스가 아예 없었고 모든 후보가 조용히 B로
떨어졌습니다. 지금은 `bash deploy/herald-deploy.sh`의 4단계
(`deploy/herald-copy-corpus.sh`)가 `items.json`·`runs.json`·`state.json`을 배포 시점에 복사합니다 —
스티어링 설정과 같은 "배포 시점 사본" 원칙이고, 이유도 같습니다(프로덕션이 개발 트리의 살아 있는
파일을 읽으면 안 됩니다). 즉 **코퍼스를 새로 모았으면 배포도 한 번 돌려야 스케줄러에 닿습니다.**
