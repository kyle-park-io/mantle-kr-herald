...
- **만드는 법** — `pnpm auth:hash`. 비밀번호를 인자로 받지 않습니다(셸 히스토리·프로세스 목록에 남지 않게) — 프롬프트로 입력받고 붙여넣을 두 줄을 출력합니다. 12자 미만이면 거부합니다
...
보여줍니다 (`web/src/App.tsx:191`). §3의 조용한 축소를 잡는 데는 이게 제일 빠릅니다 — 발행 타깃
