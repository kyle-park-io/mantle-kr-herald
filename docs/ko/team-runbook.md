...
### 영상에 미리보기가 안 뜰 때 — `pnpm x:video-backfill`

**증상** — 검수 화면에서 어떤 글의 영상 마커가 `[영상]`으로만 남아 있고, 마우스를 올려도 재생되지
않습니다. 사진은 정상입니다.

**원인** — 그 글의 저장된 마커에 영상 주소가 없습니다. 마커에 주소를 싣기 시작한 것이 2026-08-08이라
**그전에 번역·렌더링으로 저장된 글은 앞으로도 계속 주소가 없습니다** — 저장된 글을 다시 만들어 내는
장치는 없기 때문입니다. 이건 고장이 아니라 원래 그런 것이고, 조치할 대상이 아닙니다.

조치할 대상은 다른 쪽입니다. **수집된 원본(`x_threads`) 쪽에 주소가 비어 있는 경우**입니다.
보통은 재수집(`pnpm collect --since 70d`)이 채워 주지만, **계정 타임라인 검색에서 빠진 글에는 어떤
`--since`로도 닿지 못합니다** — 그런 글은 예전에 스레드 문맥으로만 딸려 들어온 것이라, 검색에도
그 글의 답글에도 나오지 않습니다(§4 "수집에 구멍이 생겼을 때"와 같은 부류입니다).

**조치** — ID로 직접 조회해 채웁니다. 먼저 미리보기로 무엇이 걸리는지 봅니다:

```bash
pnpm x:video-backfill
# 계획을 확인한 뒤
pnpm x:video-backfill --yes
```

`--yes` 없이는 아무것도 쓰지 않고 `--yes`와 **똑같은 계획**만 보여 줍니다. 채울 후보가 하나도 없으면
API 호출조차 하지 않으므로, 확인만 해 보는 데는 비용이 들지 않습니다. 채워진 뒤 그 글을 새로
번역하면 마커에 주소가 실립니다.
