...
6. **1차 검수** — 검수자가 `pnpm serve`를 띄우고 대시보드의 번역 검수 모드에서 각 항목을
   ...
   로그인에 성공하면 세션이 12시간 동안 유지되므로, 그 안에는 다시 로그인할 필요가 없습니다.
...
- **대시보드 비밀번호 교체 (필요할 때만 — 유출 의심, 담당자 변경 등).** `pnpm auth:hash`를
  ...
  **이미 로그인돼 있던 세션은 곧바로 끊기지 않습니다** — 세션은 비밀번호가 아니라
  `HERALD_SESSION_SECRET`으로 서명되므로, 이미 로그인한 사람은 세션이 자연 만료(12시간)될 때까지
...
**원인** — 세션이 끝났습니다. 가장 흔한 원인은 로그인한 지 12시간이 지나 세션이 자연 만료된
...
검수 · 번역, 2차 검수 · 채널)로 돌아갑니다. **단, 입력창에 아직 저장하지 않은 수정 내용은
로그인 화면으로 튕기는 순간 사라집니다** — 승인·저장은 그때그때 하는 편이 안전합니다.
