...
7. **변환 준비 → 변환 → 저장** — 운영자가 `pnpm convert:prepare`로 워크시트를 만들고,
   에이전트가 X/공지/카톡 공지/해설/소통/KOL/PR 타입별로 채우고, `pnpm convert:save --id <id> --type <t> --file <f>`로
   저장합니다.
...
(X/공지/카톡 공지/해설/소통/KOL/PR) 변환 규칙은 `conversion/<타입>.md`를 따릅니다
...
**이 증상은 머신마다 다릅니다 — 두 번째 운영 노트북(`DESKTOP-82HHK12`)에서는 재현되지
않습니다.** 2026-08-17에 같은 진단을 그 기기에서 돌리니 세 시계가 초 단위로 일치했고, **원인
쪽이 아예 성립하지 않았습니다.** 위 표의 두 주체 중 하나가 없기 때문입니다: `systemd-timesyncd`는
설치돼 있지도 않고(`systemctl is-enabled`가 `not-found`), 대신 chrony가 도는데 그쪽이 물고 있는
소스는 `ntp.ubuntu.com`이 아니라 **PHC0 — Hyper-V가 게스트에 내주는 호스트 시계 그 자체**입니다
(stratum 1, 오프셋 1.6µs). 즉 WSL2 호스트 동기화와 chrony가 **같은 시계를 가리키고 있어 다툴
상대가 없습니다.** 게다가 chrony는 부팅 때 `Could not step system clock`을 남깁니다 — 이 게스트에는
시계를 뒤로 점프시킬 수단 자체가 없다는 뜻이고, 8분 창을 만드는 것이 바로 그 역방향 점프입니다.
결과도 그대로여서, 그 기기의 저널은 8분이 아니라 **석 달치**(2026-05-14 부팅부터)가 남아 있고
최근 24시간 역방향 계단은 0건입니다.

그러니 새 머신에서는 고치기 전에 **게스트 쪽 시각 소스가 무엇인지부터** 보세요:

```bash
systemctl is-enabled systemd-timesyncd   # not-found면 위 표의 두 번째 주체가 없는 기기입니다
chronyc sources                          # PHC0면 호스트를, ntp.*면 인터넷을 물고 있습니다
chronyc tracking                         # 그 소스와의 실제 오프셋
```

**그 기기에서도 `w32time`은 STOPPED/DEMAND_START입니다** — 설정은 똑같은데 증상만 없습니다. 위
PowerShell 세 줄은 그대로 유효하되 지금 할 일은 아니고, 대신 **고장 모양이 다르다**는 것을 알아
두세요. 시계 주인이 하나뿐인 기기에서 호스트가 틀어지면 게스트는 점프 없이 **매끄럽게 같이**
틀어집니다: 로그가 사라지지도, 테스트가 깨지지도 않고 절대 시각만 조용히 어긋나므로 위 "증상"
세 가지 중 어느 것도 신호가 되어 주지 않습니다. 그 기기에서 시계를 의심할 근거는 위 진단
세 줄뿐입니다.
...
- **`.env`를 고치고 배포를 안 돌렸는지는 `pnpm doctor`의 `.env deploy sync` 줄이 말해 줍니다.**
  아래 스티어링 줄과 같은 성질의 함정이고, 자격 증명 쪽이 더 조용합니다: 키를 로테이션하고 이 스크립트를
  안 돌리면 타이머는 **옛 키로 계속 인증**하고, 나중에 터졌을 때 증상은 "자격 증명이 잘못됐다"로 보여서
  사본이 낡았다는 쪽을 아무도 의심하지 않습니다. 이 줄은 어긋난 **변수 이름만** 출력합니다
  (`+`는 이쪽에만, `~`는 값이 다름, `-`는 배포 트리에만 — 배포 게이트와 같은 방향).
  배포 트리에 `.env`가 아예 없으면 `warn`이 아니라 `fail`입니다. 여섯 타이머가 전부 그 파일을 읽으므로
  스케줄러가 아예 못 돕니다.
...
- **로그** — `ls ~/.herald/logs/herald-watch/` 로 실행 목록을, `cat` 으로 그 실행 하나를 봅니다.
  ...
  **단, 이건 이 문단을 쓴 기기 이야기입니다.** 두 번째 운영 노트북에서는 저널이 몇 달치 남아
  있어 `journalctl --user -u herald-convert`로 어젯밤 tick까지 그대로 읽힙니다 — 내 기기가 어느
  쪽인지는 §4 "로그가 사라지거나 테스트가 이유 없이 깨질 때 — 시계"의 진단이 가릅니다. 어느
  기기든 `~/.herald/logs/`는 항상 답을 갖고 있으니, 습관은 그쪽으로 들이는 편이 안전합니다.
...
**항목 하나는 아직 변환되지 않은 유형 수만큼(최대 7개: X·공지·카톡 공지·해설·소통·KOL·PR) 부풀고,
그 전부를 `claude -p` 한 번이 씁니다.** 즉 이 값은 10분 상한에 닿기 전에 7배로 곱해집니다.
...
#### 한 tick이 에이전트에게 넘기는 유형은 최대 4개입니다

`HERALD_CONVERT_BATCH`가 **항목** 수를 세는 것과 달리, 이쪽은 그 항목들이 부풀어난 **(항목, 유형)
쌍**을 셉니다 — `claude -p` 한 번이 실제로 써야 하는 원고 수이고, 10분 상한이 걸리는 대상입니다.
`convert:prepare --max-variants`로 넘어가며 **환경변수가 아니라 코드 상수**입니다
(`MAX_VARIANTS_PER_TICK`, `src/app/ConvertTick.ts`). 취향이 아니라 상한에 대한 산수라서 그렇습니다.

**왜 필요했는지 — 2026-08-17.** 항목 하나(`x:2088271949462138999`)가 7종으로 부풀었고, 에이전트가
5종(`x`·`공지`·`카톡 공지`·`해설`·`소통`)을 저장한 뒤 `kol`·`pr` 작업 중에 상한에 걸려 tick이
`exit 1`로 끝났고 **텔레그램 알림이 한 통** 나갔습니다. 워크시트의 `.ko.txt` 파일 시각으로 잰
속도는 **첫 유형이 나오기까지 약 5분**(205KB짜리 워크시트를 읽는 구간 — 워크시트 크기는 유형
섹션 수에 대체로 비례합니다), **그 뒤로는 유형당 약 1분**이었습니다. 7종이면 12분 안팎이라
10분 상한을 넘습니다. `--limit`으로는 막을 수 없었습니다 — 이미 1이었으니까요.

**넘긴 쌍은 버려지지 않고 미뤄집니다.** `convert:prepare`는 "변환본 행이 없는" 쌍을 고르므로,
이번에 잘린 쌍은 30분 뒤 다음 발사에서 그대로 다시 선택됩니다. 7종짜리 항목은 두 tick에 걸쳐
변환되고, 대가는 변환 하나가 아니라 **30분의 지연**입니다.

**상한(10분) 쪽을 올리는 건 답이 아닙니다.** `TimeoutStartSec=`는 발사 주기의 **절반(900초)**을
넘을 수 없고(`tests/deploy/convertTiming.test.ts`가 못박습니다 — 넘기면 wedged tick 하나가 다음
발사를 통째로 건너뛰게 만듭니다), 그 안쪽에 있어야 하는 `ClaudeCodeAgent`의 10분은 기껏해야 1분
남짓밖에 못 늘립니다. 12분은 그 안에 들어오지 않습니다. 발사 주기 자체는 Neon 요금 결정이고요.

**그리고 상한에 걸리더라도 그때까지 저장된 변환본은 남습니다.** 에이전트는 유형마다
`convert:save`를 부르므로 저장은 이미 프로덕션에 반영돼 있습니다 — 위 사고에서도 5종은 남았고,
다음 tick의 `pending.json`은 7건이 아니라 2건이었으며, 그 tick은 6분 만에 `exit 0`으로 끝나면서
`format`까지 돌아 2차 검수 카드 7장을 만들었습니다. 4개 상한은 이 복구 경로를 대신하는 게 아니라,
**평범한 항목 하나가 그 경로를 밟을 일 자체를 없앱니다.**

**`output/variants/variants.json`이 없다고 "하나도 저장 못 했다"고 읽지 마세요.** 스케줄된 tick은
`HERALD_DB_ENV=production`으로 돌아 변환본을 **프로덕션 Neon에 씁니다** — 그 파일은 이 머신에 아예
생기지 않습니다(위 사고를 진단하면서 실제로 한 번 잘못 읽었습니다). 무엇이 저장됐는지는 보드(2차
검수 화면)나 프로덕션 환경의 `pnpm status`가 답합니다:

```bash
set -a; . ~/.herald/prod.env; set +a
HERALD_OUTPUT_DIR=$HOME/.herald/output pnpm status   # Converted (variants) 줄
```
...
먼저 사람이 읽어야 합니다). **켜기 전 확인할 두 가지 사전 조건**, 둘 다 사람이 손으로 채우는 항목이라
`enable`이 대신 검사해 주지 않습니다:

1. **로스터** — `KOL list`의 `Social media link`가 채워진 행만 스윕 대상입니다(위 `kol-roster-migrate`
   계획 참고). 비어 있으면 이 유닛은 조용히 아무 채널도 훑지 못한 채 매주 성공 종료합니다.
2. **월별 탭의 수식·서식** — `Jul.`/`Aug.`/`Sep.` 각 탭의 로그 영역에서 세 수식 컬럼(`Engagement
   ...
   추가할 때마다(요약 블록이 자라 로그 헤더 행이 밀릴 수 있으므로) 사람이 직접 해야 합니다.
...
`herald-x-reconcile`는 바로 아래, `herald-creds`는 아래 "크레덴셜 상시 점검",
`herald-translate-check`는 아래 "용어집 주간 다이제스트"). 켜기 전에 손으로 한 번 돌려 보는
게이트가 유닛마다 다르기 때문에 한 줄로 묶지 않았습니다 — 그중 가장 단순한 것이
`herald-translate-check`이고(그 유닛은 파이프라인 데이터를 아무것도 쓰지 않습니다), 가장 위험한
...
1. **`pnpm translate:check --notify`** — 우리 초안이 용어집이 정한 말을 썼는지(드리프트), 그리고
   ...
   설명은 위 "발행 원문 캡처와 용어집 대조" 절에 있습니다.
2. **`pnpm glossary:mine --notify`** — 앞의 둘 중 어느 쪽도 물을 수 없는 질문. **아직 결정된 적이
   없는 용어**를 찾습니다. 결과는 검토 파일 하나이고, 그 절대경로가 알림에 실려 옵니다.

**왜 한 유닛에 둘을 넣었나 — 다섯 번째 타이머를 만들지 않고.** 같은 사람이 같은 자리에서 읽는 한
질문의 두 쪽이고, 둘 다 같은 두 개의 느린 입력(사람이 용어집을 고치는 것, `x:reconcile`이 발행
원문을 캡처하는 것)에 매달려 있습니다. 타이머를 따로 두면 아무도 원하지 않는 독립 스케줄이 생기고
주간 방해가 한 번 더 늡니다. **앞 줄이 실패하면 뒤 줄은 아예 돌지 않는데, 이것도 의도된
결과입니다** — 둘 다 같은 데이터베이스와 같은 용어집을 읽으므로, 환경이 깨졌으면 한 줄 뒤에 더
헷갈리는 두 번째 알림이 오는 것보다 하나만 오는 게 맞습니다.

**왜 주 1회인가.** 나머지 네 타이머는 스스로 움직이는 것을 지켜봅니다 — 새 글이 오고, 큐가 빠지고,
크레덴셜이 죽습니다. 이쪽은 **거의 움직이지 않는 입력에서 보고서를 다시 뽑습니다.** 발행 원문은
`x:reconcile`이 살아 있는 스레드를 매칭했을 때만 채워지니 하루 최대 네 번, 실제로는 한국어 계정이
글을 올렸을 때뿐이고, 용어집은 사람이 고치는 파일이며 그 편집은 누가 `herald-deploy.sh`를 돌리기
전까지 배포 체크아웃에 닿지도 않습니다.

게다가 **두 보고서 모두 누적이지 델타가 아닙니다.** 커서도 "봤음" 상태도 없이 매번 원장 전체를
다시 읽으므로, 사람이 번역을 고치거나 용어집을 바꾸거나 기각을 기록하기 전까지 모든 발견 항목이
계속 발견 항목으로 남습니다. 하루 한 번이면 같은 목록이 일주일에 여섯 번 더 옵니다 — 2026-08-07의
번역 19건(결정 용어 4개)도, 2026-08-11 첫 발굴의 후보 10건도 누가 손댈 때까지 매일 아침 다시
왔을 겁니다. 하루 한 번을 고른 위 `herald-creds`의 논리에서 한 걸음 더 간 것이고, 여기서는 그럴
이유가 더 분명합니다: **이 보고서에 담기는 건 장애가 아닙니다.** 아무것도 막혀 있지 않고, 기다리는
동안 나빠지는 것도 없으며, 어차피 1차 검수 전에 한 번에 몰아 읽습니다.

한 달에 한 번은 반대로 너무 깁니다. 이 보고서의 목적은 **더 나가기 전에** 드리프트를 잡는 것이고,
한 달치 안 읽힌 드리프트는 그걸 그대로 실어 나른 한 달치 게시물입니다.

**왜 월요일 06:53인가.** 월요일인 건 이 다이제스트가 그 주 검수 작업의 읽을거리이기 때문입니다 —
주중에 끼어드는 것보다 주가 시작될 때 놓여 있는 게 맞습니다. 06:53은 실제 작업 시간대
(KST 10:00~04:00) 바깥의 유일한 조용한 틈이고, `herald-creds`의 06:23이 같은 이유로 고른
자리입니다. **시각도 임의가 아닙니다** — 06:53은 06:41 `x:reconcile` 발화보다 12분 뒤라, 그 주가
마지막으로 캡처한 발행 원문이 데이터베이스에 들어온 **다음에** 이 유닛이 읽습니다(그 발행 원문은
`glossary:mine`의 치환 신호, 즉 사람이 직접 준 답이기도 합니다).

> **조용한 주가 정상입니다 — 그리고 그게 이 유닛의 유일한 함정입니다.** 두 명령 모두 보고할 게
> 없으면 **알림을 아예 보내지 않습니다**(오버라이드가 없으면 `overrideNotification`이, 후보가
> 없으면 `miningNotification`이 각각 아무것도 반환하지 않습니다). 즉 침묵은 "다 괜찮다"가 아니라
> "할 말이 없다"이고, **깨끗한 한 주와 죽은 타이머가 겉으로 구분되지 않습니다.** 그 둘을 가르는
> 건 `OnFailure=herald-notify-failure@%n.service` 하나뿐인데, 주 1회 주기에서는 그 침묵이 이레씩
> 갑니다. 알림 없는 주가 이어지면 `systemctl --user list-timers`로 다음 발화 시각을 한 번
> 보세요 — 이 유닛에서는 그게 값을 합니다.

**사람이 관리하는 것은 하나도 바꾸지 않고, 아무것도 막지 않습니다.** 두 명령 다 데이터베이스 행을
쓰지 않고 상태를 바꾸지 않으며, **발견 항목이 있어도 종료 코드가 0입니다** — 용어집에는 진짜 예외가
있고 결정되지 않은 용어는 오류가 아니라 질문이니, 이건 게이트가 아니라 1차 검수 전에 사람이 읽는
목록입니다. 유용한 부작용이 하나 따라옵니다: 이 유닛의 0 아닌 종료는 **언제나 실행 자체가
깨졌다는 뜻**(데이터베이스 없음, 용어집 없음, 타임아웃)이지 보고서에 뭔가 있었다는 뜻이 아닙니다.
그래서 위 실패 알림은 주간 오탐이 될 수 없습니다. 드리프트 쪽은 알림으로 나가지도 않습니다 —
그건 실행 로그에서 읽습니다.

**쓰는 것은 검토 파일 하나뿐입니다.** 이 유닛은
`Environment=HERALD_OUTPUT_DIR=%h/.herald/output`을 받으므로 경로는
`~/.herald/output/glossary/candidates-<날짜>.json`이지, 개발 트리의 `output/` 아래가
**아닙니다.** 손으로 돌린 `pnpm glossary:mine`이 쓰는 경로와 다르기 때문에 그 절대경로가 표준
출력 마지막(`review file: …`)과 ops 알림 마지막 줄에 함께 실립니다 — "초안을 열어 보세요"라고만
하고 어느 초안인지 말하지 않는 알림은 함정이니까요.

**참조 코퍼스는 이 유닛이 모으지 않습니다.** `glossary:mine`의 교차 검증은
`~/.herald/output/x/reference/`의 @0xMantleKR 코퍼스를 읽는데, 거기에 쓰는 건 **손으로 돌리는
`pnpm collect:reference`뿐입니다**(주마다 수집하면 압도적으로 과거 데이터에 twitterapi.io 예산을
씁니다). 28일이 지나면 알림과 로그에 `⚠ 참조 코퍼스 … 지남` 줄이 붙고 등급이 전부 B로
떨어집니다 — 조용히 고치는 대신 그렇게 말합니다. 고치는 건 **두 단계**입니다:

```bash
pnpm collect:reference               # 개발 체크아웃의 output/x/reference/ 아래에 씁니다
bash deploy/herald-deploy.sh         # 4단계가 그걸 ~/.herald/output 아래로 옮겨 줍니다
```

2026-08-11 첫 실제 발화가 정확히 이 구멍에 빠졌습니다: 코퍼스가 그 자리에 아예 없어서 교차 검증이
통째로 눈이 멀었고, 후보 170건이 전부 B로 나왔습니다. 배포 4단계
(`deploy/herald-copy-corpus.sh`)가 그 결과로 붙었습니다 — 스티어링 설정과 같은 이유입니다:
프로덕션은 누가 편집하는 트리의 파일을 직접 읽으면 안 됩니다. (후보가 지나치게 길었던 것도 그 첫
실행인데, 대부분 줄머리 대문자 낱말이었고 지금은 위치 규칙이 걸러 냅니다 — 몇 개를 걸렀는지는
로그의 `dropped … capitalized word(s)` 줄과 검토 파일 머리말에 적힙니다.
[`capabilities.md` §6](capabilities.md#용어집-후보-발굴--pnpm-glossarymine---notify))

**로그는 한 발화에 두 파일입니다.** 래퍼(`herald-run-logged.sh`)가 **프로세스마다** 타임스탬프를
새로 찍으므로, `ExecStart=` 두 줄은 `~/.herald/logs/herald-translate-check/` 안에 각자의
`<타임스탬프>.log`를 남깁니다. 같은 발화라 몇 초 차이로 이름순 나란히 붙고, systemd가 유닛
**시작**마다 하나씩 주는 `invocation` id가 같아서 한 발화의 짝으로 묶입니다. 보관 상한 60개는 유닛
디렉터리 단위이므로 이 유닛의 역사는 약 30발화 — 주 1회로 반년이 넘습니다. 앞 명령이 실패하면 뒤
명령은 돌지 않으므로, 그때는 **이름순 마지막 파일이 곧 실패한 실행**입니다.

**`TimeoutStartSec=840`은 `Type=oneshot`이라 두 줄 전체를 덮습니다** — 줄마다가 아닙니다.
120+300+120+300이고, 명령마다 데이터베이스 몫 120초와 텔레그램 몫 300초입니다. 300초는
`notifyOps`가 타임아웃 없이 `fetch`를 부르는 탓에 undici의 헤더 타임아웃이 실질 상한이라서 나온
숫자이고, **그보다 낮게 잡으면 전송 도중에 SIGTERM이 납니다** — 이미 끝나서 로그에 다 찍힌 점검을
...
않고 발견 항목이 있어도 0으로 끝나므로, `systemctl --user start herald-translate-check.service`
자체가 곧 "확인용 실행"입니다 — `--yes`가 붙은 `herald-x-reconcile`와 정반대입니다:

```bash
bash deploy/herald-deploy.sh                             # 유닛 복사 전에 먼저 — 위 "순서 주의" 참고
systemctl --user start herald-translate-check.service    # Type=oneshot이라 끝날 때까지 붙잡습니다
systemctl --user show herald-translate-check.service -p Result -p ExecMainStatus
ls -1 ~/.herald/logs/herald-translate-check/ | tail -2    # 한 발화에 두 파일입니다
```

로그 끝의 `review file: …` 줄과 ops 방에 온 알림을 확인한 뒤 켭니다. 후보가 하나도 없으면 알림이
오지 않는 것이 정상이고, 그때도 검토 파일과 로그 줄은 남습니다.

```bash
systemctl --user enable --now herald-translate-check.timer
```

타이머는 월요일 06:53에 주 1회만 돌므로, **켠 직후에 아무 일도 일어나지 않는 것이 정상입니다.**
확인·정지 명령은 위 watch 스케줄러와 같고 유닛 이름만 `herald-translate-check`로 바꾸면 됩니다
(`systemctl --user list-timers`, `journalctl --user -u herald-translate-check`,
`ls ~/.herald/logs/herald-translate-check/`). 실패 알림 저널은
`journalctl --user -u herald-notify-failure@herald-translate-check.service.service`입니다.
