...
**원인** — 로그인 실패 5회는 계정 하나를 60초 동안 잠그는데, 이 잠금은 아이디·비밀번호별이
아니라 **계정 전체에 하나뿐**입니다(공유 계정이 하나뿐이므로). 그래서 누군가(팀원의 오타든, 그
계정을 노린 외부 요청이든) 60초마다 한 번씩 틀린 시도를 계속 흘려보내면 잠금이 끊임없이
갱신되어 팀 전체가 로그인하지 못하는 상태가 이어질 수 있습니다. 예전에는(서버 재시작 시
초기화되던 시절) 서버를 껐다 켜면 이 상태가 풀렸지만, **지금은 이 잠금 기록이 데이터베이스에
남아 서버를 재시작해도 풀리지 않습니다** — 여러 인스턴스·재배포에도 잠금이 유지되도록 일부러
이렇게 만들었습니다.

**조치** — 서버 재시작으로는 풀리지 않으니, 데이터베이스에서 직접 잠금 기록을 지워야 합니다.
`.env`의 `DATABASE_URL`로 접속해서 다음을 실행하세요:

```bash
psql "$DATABASE_URL" -c "delete from auth_attempts where id = 'singleton';"
```

행 하나만 지우는 명령이라 되돌릴 필요가 없고, 실행하는 순간 실패 횟수와 잠금이 모두
초기화되어 바로 다시 로그인을 시도할 수 있습니다. 이 상황이 반복되면(특정 시간대에 계속
잠긴다든지) 외부에서 계정을 노리고 있다는 신호일 수 있으니 §2의 "대시보드 비밀번호 교체"로
비밀번호를 바꾸는 것도 고려하세요.
