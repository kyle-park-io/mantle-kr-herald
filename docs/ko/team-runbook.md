...
**조치** — `pnpm drive:publish`를 실행하면 `unsynced` 항목(원장에 아직 행이 없는 항목)만 새로
업로드됩니다. **`stale` 항목에는 자동 복구 수단이 없습니다.** `PublishTranslations`는 `(itemId,
status, target)` 키가 이미 원장에 있으면 그 항목을 무조건 건너뛰고
(`published.has(key)` 체크, `src/app/PublishTranslations.ts`) `contentHash`나 `isStale`을 전혀
들여다보지 않습니다 — `contentHash`는 `pnpm status`가 "승인 후 수정했지만 Drive에는 옛 버전이
남아 있다"는 상태를 *감지*하도록 만들어진 값일 뿐, 재업로드를 트리거하는 데 쓰이지 않습니다. 즉
같은 명령을 몇 번을 다시 실행해도 `stale` 항목은 그대로 남습니다 — 이것은 아직 구현되지 않은
알려진 한계이며, 정상적인 운영 절차가 아닙니다.

`stale` 항목을 실제로 갱신하려면 수동 우회가 필요합니다.

1. `output/publish/state.json`(동기화 원장)을 열어 해당 항목의 행을 지웁니다 — 행은 `itemId` /
   `status` / `target` 조합으로 식별됩니다([`artifacts.md`](artifacts.md) §4).
2. `pnpm drive:publish`를 다시 실행합니다. 원장에 그 항목의 행이 더 이상 없으므로 이번에는
   `unsynced`로 취급되어 현재 내용으로 새로 업로드됩니다.
3. **주의 — 이전 파일이 Drive 폴더에 그대로 남습니다.** `GoogleDriveUploader`/`LarkDriveUploader`는
   기존 파일을 갱신(PATCH)하는 게 아니라 항상 새 파일을 생성(POST)합니다. 따라서 이 절차는 옛
   파일을 덮어쓰는 것이 아니라 같은 이름의 파일을 하나 더 만드는 것이고, 옛 사본은 Drive에서 직접
   찾아 수동으로 지워야 합니다.
