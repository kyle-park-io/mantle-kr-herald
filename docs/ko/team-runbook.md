...
에이전트가 항목·유형마다 `pnpm convert:save`를 부릅니다. 대부분의 실행은 승인 대기분이 없어서
아무것도 하지 않고 끝나며, 그럴 때는 워크시트도 만들지 않고 에이전트도 부르지 않습니다.
...
`status: "converted"`이고, 거기서 2차 검수를 기다립니다 — `pnpm format`도 `pnpm send:*`도 `pnpm
drive:publish`도 이 경로에서는 호출되지 않습니다. 에이전트에게 열려 있는 셸 명령은 `pnpm
convert:save` 하나뿐이고, 발송 계열 명령은 권한 규칙에서 **거부**로 못박혀 있습니다.
