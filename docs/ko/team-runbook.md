...
**Google Drive와 로컬 대상에서만 동작합니다.** Google과 `local`(`LocalFileUploader`)은 둘 다
파일을 그 자리에서 바꾸는 `update()`를 구현하지만, Lark Drive `drive/v1`에는 내용을 그 자리에서
바꾸는 엔드포인트가 없으므로, Lark 대상의 `stale` 항목은 자동 갱신되지 않고 해당 항목을 지목하는
...
달라진 `stale` 행이라면, 대상이 Google이거나 `local`인 경우에 한해 절대 쓰지 마세요.** Google과
`local` `stale` 행은 위에서 설명한 자동 경로가 이미 처리합니다. 원장 행을 지우고 재게시하면 그
경로를 건너뛰고 `update()`가 아니라 `upload()`로 완전히 새로 게시하게 되어, Google은 새 파일
id가 발급돼 기존 공유 링크가 끊기고, `local`은 그 사이 `publishFileName`이 바뀌었다면(예:
재승인으로 `approvedAt` 날짜가 바뀐 경우) 옛 파일이 원장에서도 디스크에서도 참조를 잃고 그대로
남아 고아 중복 파일이 됩니다 — `pnpm archive`도 `pnpm clean`도 `output/publish/local/`은 청소
대상으로 보지 않으므로([`artifacts.md`](artifacts.md) §2) 아무도 지우지 않습니다. (Lark `stale`
행은 자동 경로 자체가 없으므로 위 ②번이 정당한 선택지입니다 — 그래서 이 금지는 Google과
`local`에만 적용되고, Lark에는 적용되지 않습니다.) 정리하면 판단 기준은 두 가지입니다:
**`contentHash`가 있는가**, 그리고 **대상이 Google 또는 `local`인가.** 둘 다 예일 때만 손대지
말고 자동 경로에 맡기세요 — 그 밖의 경우(`contentHash`가 아예 없는 레거시 행, 또는 대상이
Lark인 `stale` 행)에만 이 수동 절차를 쓰세요.
