...
**조치** — `--since`만 붙여 그냥 다시 돌리면 **똑같은 지점에서 똑같이 잘리고 아무것도
...
건드리지 않습니다. 실행이 끝나면 성공을 가정하지 말고 `output/x/runs.json`의 마지막 실행
행에서 `truncated`/`gap`을 직접 확인하세요 — 거기 `gap`이 또 찍혀 있으면 상한을 더 올려서
다시 돌려야 합니다.

**참고** — 워터마크가 이미 전진해 있으므로 다음 예정된 tick은 구멍을 채웠는지와 무관하게
그냥 초록으로 끝납니다. 초록 tick이 구멍이 메워졌다는 증거는 아닙니다.
