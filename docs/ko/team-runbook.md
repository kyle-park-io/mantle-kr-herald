      ...
      **하나만** 올라갑니다(예전에는 예약분과 새 글이 둘 다 올라갔습니다). 취소가 실제로 취소였는지
      ...
      - `… 취소하는 사이에 원본이 게시된 것으로 보입니다 …` — **취소가 한 발 늦은 경우입니다.**
        ...
        - 그때 게시를 기다리던 예약이 이 글뿐이었다면 메시지가 그렇게 적힙니다 — 사실상 이 글이
          ...
          예약해 둔 초안이 그때 올라갔다면 그쪽일 수도 있습니다. 조치: 계정에서 방금 올라간 글을
          확인하세요.
        - 같이 기다리던 예약이 더 있었다면(`… 이 글 말고도 N건 …`) 쿼터는 **그중 다른 글** 때문에
          줄었을 수도 있습니다. 조치: 계정을 확인하고, 이 글이 실제로 올라가지 않았다면 `재발송`을
          한 번 더 누르세요 — 그때는 그대로 나갑니다.
      - `… 취소 요청은 받아들여졌지만, 그 사이 원본이 게시됐는지는 확인하지 못했습니다 (월간 발행
        쿼터를 읽지 못했습니다) …` — 위와 같은 확인을 **쿼터를 못 읽어서** 하지 못한 경우입니다.
        올라갔는지 아닌지 모르므로, 안전한 쪽(올라갔을 수 있다)으로 멈춥니다. **아무것도 새로 나가지
        않았습니다.** 조치: 계정을 확인하고, 이 글이 실제로 올라가지 않았다면 `재발송`을 한 번 더
        누르세요.
