    ...
    **재발송(보드의 빨간 `재발송` 버튼)은 줄의 상태에 따라 하는 일이 다릅니다.** 정기 단계가 아니라
    ...
    - **`예약됨` 줄(아직 게시 전)** — **예약된 원본을 먼저 취소하고 새로 보냅니다.** 그 방에는 글이
      **하나만** 올라갑니다(예전에는 예약분과 새 글이 둘 다 올라갔습니다). 취소가 실제로 취소였는지
      ...
      - `… 취소하는 사이에 원본이 게시됐습니다 …` — **취소가 한 발 늦은 경우입니다.** Typefully는
        예약을 취소했을 때와 **이미 올라간 글의 초안 기록을 지웠을 때 똑같은 응답**을 주므로, 응답만
        보고는 둘을 구분할 수 없습니다. 그래서 취소 전후로 **월간 발행 쿼터**를 읽어 비교하고, 한 칸이
        줄었으면 그 사이에 무언가 게시된 것으로 봅니다. **아무것도 새로 나가지 않았습니다.**
        - 그때 게시를 기다리던 예약이 이 글뿐이었다면 메시지가 그렇게 적히고, **이 글이 올라간 게
          맞습니다.** 조치: 계정에서 방금 올라간 글을 확인하세요.
