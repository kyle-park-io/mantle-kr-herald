...
**이 수동 절차는 `contentHash`가 아예 없는 행에만 적용하세요 — `contentHash`가 있는데 값만
달라진(`stale`) 행에는 절대 쓰지 마세요.** 그런 행은 대상이 Google이면 위에서 설명한 자동
경로가 이미 처리해 파일 id와 공유 링크를 그대로 유지합니다. 원장 행을 지우고 재게시하면 새
파일 id가 발급되어 그 보장이 깨지고 불필요한 중복까지 생기므로, 두 경우를 헷갈리면 안 됩니다.
