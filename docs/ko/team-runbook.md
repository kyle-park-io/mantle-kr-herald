...
표시합니다([`artifacts.md`](artifacts.md) §4). 원장은 `target`(`google`/`lark`/`local`)마다
별도 행을 가지므로, 아래 증상·원인·조치는 `local` 행(`output/publish/local/`에 쓴 항목)에도
`google`/`lark` 행과 동일하게 적용됩니다.
