...
11. **예약 확인 — `pnpm send:reconcile`.** X로 나간 글은 즉시 게시되지 않습니다. X가 링크 있는 글의
    ...
    **재발송(보드의 빨간 `재발송` 버튼)은 줄의 상태에 따라 하는 일이 다릅니다.** 정기 단계가 아니라
    ...
    - **`예약됨` 줄(아직 게시 전)** — **예약된 원본을 먼저 취소하고 새로 보냅니다.** 그 방에는 글이
      **하나만** 올라갑니다(예전에는 예약분과 새 글이 둘 다 올라갔습니다). 대신 안전을 위해 **두
      경우에는 아예 보내지 않고 거절**합니다.
...
### 발송이 `Timed out … acquiring the lock on …`으로 멈췄을 때

**증상** — `pnpm send:channels`(또는 `pnpm send:x-article`, 보드의 `발송`)가 다음과 같은 오류로
끝납니다.

```
Timed out after 45000ms acquiring the lock on <경로>/deliveries.json (held by pid 12345).
Refusing to write unprotected: a concurrent read-modify-write would drop a row, and a dropped
send row becomes a duplicate live post. If no such process is running, remove <경로>/deliveries.json.lock.
```

**원인** — 발송 원장은 여러 프로세스가 동시에 고쳐 쓰지 못하도록 `<파일>.lock`을 잡습니다. 이
오류는 **다른 프로세스가 지금 그 원장을 쓰고 있다**는 뜻입니다 — 거의 항상 **다른 터미널의
`pnpm send:channels`**이거나 **`pnpm serve`(대시보드는 2분마다 예약 확인을 돌리며 원장을
씁니다)**입니다. 잠금이 없으면 두 프로세스가 같은 배열을 읽어 각자 쓰고, 나중에 저장한 쪽이 앞선
행을 지웁니다 — 사라진 행은 "안 보냈다"는 뜻이 되어 **다음 실행이 같은 글을 살아 있는 방에 다시
올립니다.** 그래서 이 명령은 잠금을 못 잡으면 그냥 쓰지 않고 멈춥니다. **아무것도 나가지
않았습니다** — 그대로 다시 실행하면 됩니다.

**조치** — 순서대로 확인하세요.

1. **다른 발송이나 `pnpm serve`가 돌고 있는지 확인하세요.** 돌고 있으면 끝나기를 기다렸다가 다시
   실행하면 됩니다. 정상 동작이고 보통 몇 밀리초면 끝납니다 — 45초를 기다렸다는 것은 상대가
   멈춰 있거나(노트북 절전, 디버거) 정말 오래 걸리고 있다는 뜻입니다.
2. **아무것도 안 돌고 있는데도 계속 같은 오류가 나면**, 프로세스가 죽으면서 잠금 파일만 남은
   경우입니다. 오류 메시지가 알려주는 `remove <경로>.lock`을 그때 실행하세요. **이 조치는 발송이
   하나도 돌고 있지 않을 때만 안전합니다** — 살아 있는 발송이 들고 있는 잠금을 지우면 방금 설명한
   행 유실이 실제로 일어납니다. 메시지의 `held by pid N`으로 그 프로세스가 아직 있는지
   (`ps -p N`) 먼저 확인하세요.
3. 사실 대부분은 기다릴 필요도 없습니다 — 잠금은 **30초 넘게 방치되면** 다음 실행이 알아서
   회수합니다. 30초 이상 기다렸다가 재실행하는 것이 손으로 지우는 것보다 항상 안전합니다.

`pnpm clean --yes`도 같은 규칙을 따릅니다: `output/` 아래의 `.lock`과 `*.tmp-…` 파일 중 **30초보다
오래된 것만** 지웁니다(§5).
...
- **월간** — `pnpm clean`으로 오래된 아카이브와 좌초된 임시 파일을 정리합니다. 기본은
  **드라이런**이라 지울 목록만 출력하고, 실제로 지우려면 `--yes`를 붙여야 합니다:
  ...
  - **⚠ 발송이나 `pnpm serve`가 돌고 있는 동안에는 `pnpm clean --yes`를 돌리지 마세요.** 이 명령은
    좌초된 임시 파일(`*.tmp-…`)과 잠금 파일(`*.lock`)도 지우는데, 둘 다 **살아 있는 발송이 지금
    쓰고 있는 파일**일 수 있습니다. 원장 쓰기가 임시 파일을 만들고 이름을 바꾸는 그 찰나에 그
    파일이 사라지면 **발송은 나갔는데 원장에는 안 남고**, 다음 실행이 같은 글을 방에 한 번 더
    올립니다. 그래서 30초보다 오래된 파일만 지우도록 안전장치를 뒀지만, **문턱은 안전망이지
    허가가 아닙니다** — 발송이 끝난 뒤에 돌리세요. 드라이런(`--yes` 없이)은 아무것도 지우지
    않으므로 언제 돌려도 안전합니다.
