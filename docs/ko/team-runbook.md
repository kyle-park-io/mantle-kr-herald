...
#### 7종짜리 새 항목의 첫 tick은 10분 상한에 걸립니다 — 그리고 그건 손실이 아닙니다

2026-08-17에 실측한 모양입니다. 항목 하나(`x:2088271949462138999`)가 7종으로 부풀었고, 에이전트가
5종(`x`·`공지`·`카톡 공지`·`해설`·`소통`)을 저장한 뒤 `kol`·`pr` 작업 중에 상한에 걸려 tick이
`exit 1`로 끝났고 **텔레그램 알림이 한 통** 나갔습니다. 워크시트의 `.ko.txt` 파일 시각으로 잰
속도는 **첫 유형이 나오기까지 약 5분**(205KB짜리 워크시트를 읽는 구간), **그 뒤로는 유형당 약
1분**이었습니다 — 7종이면 12분 안팎이라 10분 상한을 넘습니다.

**그래도 그때까지 저장된 변환본은 그대로 남습니다.** 에이전트는 유형마다 `convert:save`를
부르므로 상한이 걸린 시점까지의 저장은 이미 프로덕션에 반영돼 있고, 다음 tick의
`convert:prepare`는 **아직 변환되지 않은 유형만** 준비합니다. 실제로 다음 실행의 `pending.json`은
7건이 아니라 2건(`kol`·`pr`)이었고, 그 tick은 6분 만에 `exit 0`으로 끝나면서 `format`까지 돌아
2차 검수 카드 7장을 만들었습니다. 즉 **7종짜리 새 항목마다 알림 한 통, 그리고 다음 tick에서
자동 복구**가 현재의 정상 동작입니다.

**상한을 올리는 건 답이 아닙니다.** `TimeoutStartSec=`는 발사 주기의 **절반(900초)**을 넘을 수
없고(`tests/deploy/convertTiming.test.ts`가 못박습니다 — 넘기면 wedged tick 하나가 다음 발사를
통째로 건너뛰게 만듭니다), 그 안쪽에 있어야 하는 `ClaudeCodeAgent`의 10분 상한은 기껏해야 1분
남짓밖에 못 늘립니다. 12분은 그 안에 들어오지 않습니다. 알림 자체를 없애려면 `convert:prepare`가
**항목이 아니라 (항목, 유형) 단위로** 끊어야 하고, 그건 문서가 아니라 코드 변경입니다.
