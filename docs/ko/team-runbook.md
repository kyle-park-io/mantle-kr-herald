...
설정(`.env`·`translation/`·`conversion/`·`keys/`)을 개발 체크아웃에서 심볼릭 링크로 다시 걸고,
`pnpm db:migrate`까지 실행합니다. 전부 멱등이라 몇 번을 돌려도 안전합니다.

- **설정은 복사가 아니라 링크입니다.** 이 머신에 자격증명과 팀 스티어링 결정의 사본은 하나뿐이어야
  합니다. 사본이 둘이면 관리할 게 둘이고, 어긋나는 쪽은 항상 아무도 존재를 기억 못 하는 쪽입니다.
- **링크 목록은 하드코딩이 아니라 유도됩니다.** 개발 체크아웃에서 git이 무시하는 파일이면 전부
  걸립니다. 나중에 스티어링 파일이 하나 늘어도 다음 배포가 알아서 집어갑니다.
