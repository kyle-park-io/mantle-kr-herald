...
2. **아무것도 안 돌고 있는데도 계속 같은 오류가 나면**, 프로세스가 죽으면서 잠금 파일만 남은
   ...
   (`ps -p N`) 먼저 확인하세요.
3. 사실 대부분은 기다릴 필요도 없습니다 — 잠금은 **30초 넘게 방치되고, 그 뒤 1초 동안 아무 변화가
   없으면** 다음 실행이 알아서 회수합니다. 잠금을 든 프로세스는 살아 있는 동안 5초마다 그 파일의
   시각을 갱신하므로, "오래됐다"는 것은 이제 **그 프로세스가 멈췄다**는 뜻입니다. 31초쯤 기다렸다가
   재실행하는 것이 손으로 지우는 것보다 항상 안전합니다.

`pnpm clean --yes`도 **같은 판정**을 씁니다: `output/` 아래의 `.lock`은 31초보다 오래됐고 확인 구간에
시각이 움직이지 않은 것만, `*.tmp-…`는 **그 파일이 올라갈 원장의 잠금이 살아 있지 않을 때만** 지웁니다
(§5).
...
- **월간** — `pnpm clean`으로 오래된 아카이브와 좌초된 임시 파일을 정리합니다. 기본은
  **드라이런**이라 지울 목록만 출력하고, 실제로 지우려면 `--yes`를 붙여야 합니다:
  ...
  - **⚠ 발송이나 `pnpm serve`가 돌고 있는 동안에는 `pnpm clean --yes`를 돌리지 마세요.** 이 명령은
    ...
    올립니다. 그래서 잠금 회수와 **똑같은 판정**을 거쳐서만 지웁니다(위 §"잠금" 참고: 31초 + 시각이
    움직이지 않을 것, 임시 파일은 그 원장의 잠금이 죽었을 것). 잠금이 걸린 파일이 실제로 있으면
    `clean`이 최대 1초 기다립니다. 그래도 **문턱은 안전망이지 허가가 아닙니다** — 발송이 끝난 뒤에
    돌리세요. 드라이런(`--yes` 없이)은 아무것도 지우지 않으므로 언제 돌려도 안전합니다.
