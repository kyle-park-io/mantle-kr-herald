...
`formatForChannel` is reduced to **canonical normalisation**: trim, and collapse runs of blank
lines. The current `collapseBlankLines` replaces `\n{3,}` with `\n\n`, which would destroy every
post boundary; it becomes `\n{4,}` → `\n\n\n`, preserving exactly one boundary marker.
...
export interface EmitSegment { text: string; note?: string }
...
```
POST /api/renderings/emissions   { itemId, type, channel }
  → { x_paste: EmitResult, x_typefully: EmitResult, telegram_paste: EmitResult,
      telegram_bot: EmitResult, kakao_paste: EmitResult, pr_mail: EmitResult }
```

POST-with-body matches the existing rendering routes, which already pass identity that way because
`itemId` contains a `:`. All six are returned in one call so the copy buttons respond instantly.
`Destination` and `EmitResult` are mirrored into `web/src/types.ts` with the usual sync comment.

`RenderingDetail.tsx` gains a destination tab strip above a segment list:

```
[ X 붙여넣기 ] [ Typefully ] [ 텔레그램 ] [ 텔레그램 봇 ] [ 카카오 ] [ 메일 ]
