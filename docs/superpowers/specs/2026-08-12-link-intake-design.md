...
목록에 남은 항목은 영영 번역되지 않고, 집계는 다음 틱이 번역할 항목을 "닿을 수 없음"으로 세지만,
...
(`src/domain/translation/translateFloor.ts`). 세 곳이 그것을 부르므로 서로 다른 판정을 내릴 수가
...
부르는 쪽에 맡기지 않습니다 — 반쪽이면 세 곳이 다르게 조합할 수 있고, 함수 하나면 그럴 수가
...
import는 호스팅 번들을 그대로 둡니다. 이름이 겹치는 `src/status/translateFloor.ts`는 기준 시각을
**알아내는** 쪽이고, 이 모듈은 알아낸 값으로 **무엇을 하는지**입니다.
...
기준 시각 규칙을 겁니다(위 "규칙이 서야 하는 곳은 셋이다"). 다음 틱 시각(`*:17`)도 같이
