...
답글을 항목 본문에서 **빼므로**, 그대로 받으면 사용자가 가리킨 트윗 하나만 빠진 스레드를 넣고
...
| 링크가 스레드에 달린 답글을 가리킴 | `이 주소가 가리키는 글은 스레드에 달린 답글이라 번역 대상에서 빠집니다 — 스레드 본문 글의 주소를 넣어 주세요` |
| 정기 수집 계정의 기준 시각 이전 글(작성자를 못 읽은 글도 같음) | `이 글은 번역 기준 시각(<날짜>)보다 오래돼 자동 번역되지 않습니다 — 정말 필요하면 HERALD_TRANSLATE_SINCE를 내려야 합니다` |
