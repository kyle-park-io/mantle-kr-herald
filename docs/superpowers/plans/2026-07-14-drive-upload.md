...
- Google scope: `https://www.googleapis.com/auth/drive.file` (least privilege — SA only accesses files it created; folders are created by the SA via `drive:init`). Google token endpoint is form-urlencoded (use native `fetch`, not the JSON HttpClient).
